...
function solidBitmap(outPath: string, color: [number, number, number]): void {
	const image = createImage(64, 64, color);
	savePng(image, outPath);
}

function copyFrame(src: string, dst: string): void {
	fs.copyFileSync(src, dst);
}

/** Crea dos frames con un rectangulo desplazado 2px a la derecha. */
function shiftedRects(sequenceDir: string): void {
	const bg: [number, number, number] = [20, 20, 20];
	const fg: [number, number, number] = [220, 180, 40];
	const img0 = createImage(64, 64, bg);
	const img1 = createImage(64, 64, bg);
	for (let y = 16; y <= 36; ++y) {
		for (let x = 12; x <= 36; ++x) setPixel(img0, x, y, fg[0], fg[1], fg[2]);
		for (let x = 14; x <= 38; ++x) setPixel(img1, x, y, fg[0], fg[1], fg[2]);
	}
...
function forbiddenBlack(sequenceDir: string): void {
	const bg: [number, number, number] = [32, 96, 160];
	solidBitmap(path.join(sequenceDir, 'frame_000.png'), bg);
	const image = createImage(64, 64, bg);
	for (let y = 8; y <= 32; ++y) {
		for (let x = 8; x <= 32; ++x) setPixel(image, x, y, 0, 0, 0);
	}
...
function invokeCase(name: string, arrange: (dir: string) => void, contract: any, expectedPass: boolean, suiteDir: string): SelftestResult {
