...
function frameFingerprint(framePath: string): FrameFingerprint {
...
function comparePair(aPath: string, bPath: string, threshold: number): PairResult {
...
function buildContactSheet(frames: string[], pairs: PairResult[], outPath: string): void {
	const thumbW = 160;
	const thumbH = 128;
...
function main(): void {
	...
	let status = 'ok';
	if (expectAnimated && changedPairs.length === 0) status = 'expected_animation_but_frames_are_static';
	if (expectStatic && changedPairs.length > 0) status = 'expected_static_but_frames_changed';
