...
function main() {
	...
	if (!imagePath) fail('Uso: verify_fire.js --image <png>');
	...
	for (let y = 0; y < img.height; y += 2) {
		for (let x = 0; x < img.width; x += 2) {
			...
			if (r > b && r > 24) {
				warm++;
				if (y >= midY) warmLowerHalf++;
				tones.add(((r >> 4) << 8) | ((g >> 4) << 4) | (b >> 4));
			}
		}
	}

	if (warm < 1000) fail(`FAIL fire: muy pocos pixeles calientes (${warm})`);
	if (warmLowerHalf / warm < 0.5) {
		fail(`FAIL fire: el fuego no esta en la mitad inferior (${warmLowerHalf}/${warm})`);
	}
	if (tones.size < 8) fail(`FAIL fire: degradado pobre (${tones.size} tonos)`);
