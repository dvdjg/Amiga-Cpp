...
async function runBuild(demo) {
  ...
  await new Promise((resolve, reject) => {
    const child = spawn('powershell', [
      ...
      '-File', '.\\tools\\build\\build-demo.ps1',
      demo,
      '-DebugBuild',
    ], {
      cwd: root,
...
async function main() {
  ...
  assertOk(readyProbeSymbol !== null, 'No se encontro amg_debug_ready_probe en el map');
  ...
  try {
    ...
    assertOk(runtimeRunStatus !== null && runtimeRunStatus > 0, 'No se pudo resolver g_amg_run_status runtime');
    ...
    const stopReply = await pollSideWhile(
      protocol.waitForStop(6000),
      side,
      report.sideSamplesWhileRunning,
      1000,
    );
    ...
    assertOk(
      Math.abs(regsAtBreak.PC - runtimeReadyProbe) <= 8,
      `PC no esta cerca del breakpoint runtime: pc=${hex32(regsAtBreak.PC)} bp=${hex32(runtimeReadyProbe)}`,
    );
    ...
    assertOk(status.ok && status.magic === '0x414d4752', 'runstatus lateral debe seguir siendo legible');
