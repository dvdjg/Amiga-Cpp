...
function parseHex(v: string): number { return parseInt(v.replace(/^0x/i, ''), 16); }
...
function findMapAllocSections(mapPath: string): Array<{ start: number; size: number; end: number }> {
...
const demoName = path.basename((argValue('--demo', 'demos/amiga/274_octamed_probe') as string).replace(/\\/g, '/'));
const demoDir = path.join(root, 'out/demos', demoName);
const forced = argValue('--config', '') as string;
...
if (forced) {
  ...
  for (const e of fs.readdirSync(demoDir)) {
    ...
    if (!fs.existsSync(x)) continue;
    const mt = fs.statSync(x).mtimeMs;
    if (!best || mt > best.mt) best = { mt, cfg: e, exe: x, map: path.join(dir, `${demoName}.${e}.map`) };
  }
  if (!best) throw new Error(`Sin builds en ${demoDir}`);
  cfg = best.cfg; exe = best.exe; map = best.map;
}
const bpNames = (argValue('--bps', '_startmusic,_endmusic') as string).split(',').map((s) => s.trim()).filter(Boolean);
const holdMs = parseInt(argValue('--hold-ms', '8000') as string, 10);
...
try {
  ...
  try {
    ...
    if (text) runtimeSections = ['0x' + text];
    if (data) runtimeSections.push('0x' + data);
  } catch { /* qOffsets no soportado */ }
  for (const name of bpNames) {
    const linked = findMapSymbol(map, name);
    let addr: number | null = null;
    if (linked !== null && runtimeSections.length) addr = resolveRuntime(linked, mapSections, runtimeSections);
    report.breakpoints.push({ name, linked: linked !== null ? `0x${linked.toString(16)}` : null, runtime: addr !== null ? `0x${addr.toString(16)}` : null });
    if (addr !== null && addr > 0) {
      try { await proto.setBreakpoint('*0x' + addr.toString(16)); } catch (e: any) { report.breakpoints[report.breakpoints.length - 1].error = String(e); }
    }
  }
  ...
  while (Date.now() - start < holdMs) {
    ...
    await proto.continue();
    if (report.stops.length > 40) break;
  }
