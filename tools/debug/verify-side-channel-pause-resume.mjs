...
function findMapAllocSections(mapPath) {
  ...
  for (const line of fs.readFileSync(mapPath, 'utf8').split(/\r?\n/g)) {
    ...
    if (size > 0) sections.push({ name: match[1], start, size, end: start + size });
  }
  return sections;
}
...
async function runBuild(demo) {
  if (hasArg('--skip-build')) return;
  await new Promise((resolve, reject) => {
    const child = spawn('powershell', ['-ExecutionPolicy', 'Bypass', '-File', '.\\tools\\build\\build-demo.ps1', demo, '-DebugBuild'], {
      cwd: root, windowsHide: true, stdio: 'inherit',
    });
    child.once('exit', (code) => code === 0 ? resolve() : reject(new Error(`build-demo failed with exit code ${code}`)));
  });
}
...
class SideChannelClient {
  ...
  onData(chunk) {
    ...
    for (;;) {
      ...
      if (this.waiters.length > 0) this.waiters.shift().resolve(line);
...
async function main() {
  ...
  assertOk(runStatusSymbol !== null, 'No se encontro g_amg_run_status en el map');
  ...
  const runner = spawn('powershell', ['-ExecutionPolicy', 'Bypass', '-File', '.\\tools\\run\\run-demo.ps1', demo, '-SideChannelTimeoutMs', '6000', '-SettleMs', String(settleMs)], {
  ...
  while (!stdoutText.includes('side-channel READY') && Date.now() <= readyDeadline) {
    if (runner.exitCode !== null) throw new Error(`runner exited before READY (${runner.exitCode})\n${stdoutText}\n${stderrText}`);
    await sleep(100);
  }
  ...
  try {
    ...
    assertOk(runStatusRuntime !== null && runStatusRuntime > 0, 'No se pudo resolver g_amg_run_status runtime');
