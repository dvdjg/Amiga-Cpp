...
import { readPng, createImage, savePng, copyRect, pixel, drawText, RgbaImage } from '../lib/image.js';
...
interface PairResult {
	...
	changedCells: number;
	changedRatio: number;
	contentShiftDx: number;
	contentShiftDy: number;
...
function imageFiles(sequenceDir: string): string[] {
...
function expandVideoFrames(video: string, outDir: string, fps: number, limit: number, keep: boolean): string {
	...
	const result = spawnSync('ffmpeg', ['-y', '-i', video, '-vf', `fps=${fps}`, '-frames:v', String(limit), pattern],
		{ encoding: 'utf-8' });
	if (result.status !== 0) {
...
function colorSymbol(r: number, g: number, b: number): string {
	const luma = 0.299 * r + 0.587 * g + 0.114 * b;
	if (luma < 24) return ' ';
	if (luma > 230) return 'W';
	if (r > 180 && g > 160 && b < 120) return 'Y';
	if (r > 180 && g > 80 && b < 100) return 'O';
	if (r > 170 && g < 120 && b < 120) return 'R';
	if (g > 150 && r < 140 && b < 140) return 'G';
	if (b > 150 && r < 140 && g < 170) return 'B';
	if (g > 140 && b > 140 && r < 140) return 'C';
	if (r > 140 && b > 140 && g < 140) return 'M';
	return '.';
}

/** Localiza el rectangulo de contenido (no negro) del primer frame. */
function contentBounds(filePath: string, threshold = 8): Crop {
...
function frameMetrics(filePath: string, index: number, gridW: number, gridH: number, crop: Crop | null): FrameMetric {
	const image = readPng(filePath);
	const { width, height, data } = image;
	let sampleLeft = 0;
	let sampleTop = 0;
...
function shiftError(a: number[][], b: number[][], dx: number, dy: number): number {
	let total = 0;
	let count = 0;
	for (let y = 0; y < a.length; ++y) {
		const by = y + dy;
		if (by < 0 || by >= a.length) continue;
		for (let x = 0; x < a[0].length; ++x) {
			const bx = x + dx;
			if (bx < 0 || bx >= a[0].length) continue;
...
function compareMetrics(a: FrameMetric, b: FrameMetric, gridW: number, gridH: number, radius: number, threshold: number): PairResult {
	let totalDiff = 0;
	let changed = 0;
	const quadrants: Record<string, number> = { topLeft: 0, topRight: 0, bottomLeft: 0, bottomRight: 0 };
	const quadrantCounts: Record<string, number> = { topLeft: 0, topRight: 0, bottomLeft: 0, bottomRight: 0 };
	...
	const candidates: { dx: number; dy: number; error: number; direction: string }[] = [];
	...
	for (const candidate of [...candidates].sort((c1, c2) => c1.error - c2.error)) {
		...
		if (nearDirections.length >= 8) break;
	}

	const samples = Math.max(1, gridW * gridH);
	const roundedQuadrants: Record<string, number> = {};
...
function motionSegments(pairs: PairResult[]): any[] {
	const segments: any[] = [];
...
function decodeAmigaRunStatus(runStatus: any): any | null {
	if (!runStatus || !runStatus.ok) return null;
...
function expectedContentDirection(deltaX: number, deltaY: number): string {
...
function directionCompatible(observed: string, expected: string, candidates: string[]): boolean {
	if (expected === 'static') return observed === 'static';
	for (const direction of [observed, ...candidates]) {
		const parts = expected.split('-');
		if (parts.every((part) => direction.includes(part))) return true;
	}
	return false;
}

function amigaScrollProfile(reportPath: string, pairs: PairResult[], allowedMismatches: number): any {
	if (!reportPath) {
		return {
	...
	for (let i = 1; i < telemetry.length; ++i) {
		...
		if (!compatible) mismatches.push(observation);
	}

	let status: string;
	if (telemetry.length < 2) status = 'insufficient_telemetry';
	else if (mismatches.length > allowedMismatches) status = 'mismatch';
	else if (mismatches.length > 0) status = 'ok_with_tolerance';
	else status = 'ok';
...
function contactSheet(frames: string[], pairs: PairResult[], outPath: string): void {
	const thumbW = 192;
	const thumbH = 154;
...
function main(): void {
	...
	if (frames.length === 0) fail(`No se encontraron frames PNG/JPG/BMP en ${sequenceDir}.`);
	...
	if (profileReport !== null) {
		...
		for (const observation of profileReport.observations) {
			...
			summaryLines.push(
				`- ${mark} frames ${observation.from}..${observation.to}: cam ${observation.cameraFrom} -> ${observation.cameraTo}, ` +
