...
function uniqueSorted(values: number[], maxExclusive: number): number[] {
...
function buildProviderContent(request, sendMode) {
  ...
  const content: Record<string, any>[] = [{ type: 'text', text: lines.join('\n') }];
...
async function runOpenAiCompatibleReview(requestPath, providerPath, sendModeOverride) {
  ...
  fs.writeFileSync(
    path.join(request.outDir, `vision-review-report-${sendMode}.md`),
    [
...
function main() {
  ...
  if (args.reviewRequest) {
    return runOpenAiCompatibleReview(
      path.resolve(args.reviewRequest),
      path.resolve(args.provider),
      args.sendMode && args.sendMode !== true ? args.sendMode : ''
    );
  }
