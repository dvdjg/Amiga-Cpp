...
export type WaveKind = 'sine' | 'square' | 'triangle' | 'saw';
...
export function analyzeWave(data: Int8Array | Uint8Array, sampleRate: number): WaveReport {
	const n = data.length;
	let min = 0;
	let max = 0;
	let sum = 0;
...
export function toWav(data: Int8Array, sampleRate: number): Buffer {
