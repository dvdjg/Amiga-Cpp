 ...
 */
export function packUaf(chunks: readonly UafChunk[]): Buffer {
	let total = 8;
 ...
 */
export function parseUaf(buf: Buffer): ParsedChunk[] {
	if (buf.length < 8) {
 ...
 */
export function bitplanesFromIndexed(
	width: number,
	height: number,
	planes: number,
	pixels: Uint8Array,
...
export function paletteChunkData(colors: readonly number[]): Uint8Array {
...
export function bitplanesChunkData(
	width: number,
	height: number,
	planes: number,
	layout: number,
	...
	out[0] = (width >> 8) & 0xff; out[1] = width & 0xff;
	out[2] = (height >> 8) & 0xff; out[3] = height & 0xff;
	const rowBytes = width / 8;
	out[4] = (rowBytes >> 8) & 0xff; out[5] = rowBytes & 0xff;
	out[6] = planes & 0xff;
	out[7] = layout & 0xff;
	out[8] = 0;
	out[9] = 0;
...
export function sampleChunkData(bytes: Uint8Array): Uint8Array {
	return bytes;
}
...
function buildDemo(): Buffer {
	...
	for (let i = 0; i < 16; i++) sample[i] = (i * 8) & 0xff;
