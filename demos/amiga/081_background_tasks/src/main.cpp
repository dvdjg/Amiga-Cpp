...
struct BackgroundDemo {
	...
	void init(amiga::MinimalBackend& backend, eng::GameContext& context) {
		...
		if (!scene::compose(m_scene, backend.memory(),
				    scene::planar(kWidth, kHeight, kPlanes),
		...
		if (context.background != nullptr) {
			m_task = context.background->add(task::task_token(m_fill, &fill_bar_step), kBarRows, /*slice*/ 1u);
	...
	void update(amiga::MinimalBackend& backend, eng::GameContext& context) {
		...
		if (p.finished()) {
			...
			m_fill.row = 0;
			m_task = context.background->add(task::task_token(m_fill, &fill_bar_step), kBarRows, 1u);
		}
	}
