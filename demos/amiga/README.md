...
- **Porte 1:1 del demoscene (`079`)** — método en `docs/guides/roadmap/PORT_PROMPT_1A1.md`:
  ...
  - `116_flatshade_convex` — **IMPORTE** de `effects/flatshade-convex/flatshade-convex.c`: objeto convexo `pilka` girando con **sombreado plano por cara** (luz 0..15 por `UpdateFaceVisibility` con `InvSqrt`, sin sqrt en runtime) rellenada por `MinimalBackend::fill_triangles_blitter` (misma ruta que 078). Ver `demos/amiga/116_flatshade_convex/README.md` y `verify-116-flatshade.mjs`.
