...
- **Matemática 2D/3D (`077`)** — ports de `lib2d`/`lib3d` de demoscene (tests host HOST-010/011/013; índice `docs/demos/effects/LIBRARIES-CPP23-IMPORT-ROADMAP.md` §5):
  ...
  - `078_math3d_solid` — el mismo cubo con las caras **rellenas** (rasterizado de triángulos por scanline, relleno por tramos de byte/word) en orden lejos→cerca, con borrado del frame por **Blitter** (`FramePlan`+`CopyRect`). La malla se carga desde un **blob UAF-R** (`uaf-pack --mesh` → `src/prebuild.sh` → incbin → `Blob`+`MeshAssetView`), cerrando el ciclo exportador→runtime en 3D. El relleno por defecto es la ruta **CPU** (tramos de byte/word, verificada); incluye dos rutas **Blitter** tras `-DK_FILL_BLITTER=1` (`AmigaBackend::fill_triangles_blitter` = line+area-fill+cookie-cut, y `blit_fill_from_mask` = máscara CPU + cookie-cut Blitter). Ambas dibujan pero aún salen **rayadas** (WIP: alineación/carry de los canales A/C-D del Blitter).
- **Porte 1:1 del demoscene (`079`)** — método en `docs/guides/roadmap/PORT_PROMPT_1A1.md`:
  - `079_wireframe` — **PORTE 1:1** de `effects/wireframe/wireframe.c`: objeto `pilka` (`obj2c`) en alambre con la **línea por Blitter** (`AmigaBackend::blitter_line`, secuencia idéntica a `DrawObject`) y el modelo `Object3D` de `lib3d` portado en `eng/platform/amiga/object3d.hpp` (test HOST-014). Versión B: display 320×256×4 del engine (el original era 256×256×4) y sin doble buffer por rotación de planos; la geometría y el algoritmo son los del original. Ver `docs/demos/effects/WIREFRAME_PORT_PLAN.md`.
  - `116_flatshade_convex` — **IMPORTE fiel** de `effects/flatshade-convex/flatshade-convex.c`: objeto convexo `pilka` girando con **sombreado plano** (luz 0..15 por `UpdateFaceVisibility` con `InvSqrt`, aristas convexas por XOR, trazado `ONEDOT`+`EOR` por `AmigaBackend::blitter_line_eor` y relleno `XOR` por `blitter_area_fill`). Ver `docs/demos/effects/FLATSHADE_CONVEX_PORT_PLAN.md` y `demos/amiga/116_flatshade_convex/README.md`.
  - `080_fire_rgb` — **PORTE 1:1** de `effects/fire-rgb/fire-rgb.c`: simulación de fuego en asm (`support/fire_loop.s`) 80×64 → chunky → C2P por Blitter (13 fases) → display **HAM6** 320×256 con **cuadruplicado de líneas** por Copper (`scene::compose` con `row_repeat`, test HOST-016). El C2P se solapa en el hueco de VBlank. Ver `docs/demos/effects/FIRE_RGB_PORT_PLAN.md`.
