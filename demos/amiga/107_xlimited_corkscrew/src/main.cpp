...
//   EXTRA_DEFINES="-DK_VIEWPORT_W=288 -DK_VIEWPORT_H=224" bash ./tools/build/build-demo.sh demos/amiga/107_xlimited_corkscrew --debug --clean
//
...
//   bash ./tools/run/run-demo.sh demos/amiga/107_xlimited_corkscrew --sequence-frames 100 --sequence-interval-ms 20
//
//   # Asentamiento previo (ms reales tras READY) y warp (throughput, no suavidad):
//   bash ./tools/run/run-demo.sh demos/amiga/107_xlimited_corkscrew --settle-ms 500 --warp
//
//   # Dejar WinUAE corriendo al terminar:
//   bash ./tools/run/run-demo.sh demos/amiga/107_xlimited_corkscrew --keep-running
//
// -----------------------------------------------------------------------------
// CÓMO VERIFICAR
// -----------------------------------------------------------------------------
//   # Secuencia completa de la demo (H derecha + saveword + columna plane-shifted):
//   bash ./demos/amiga/107_xlimited_corkscrew/analyze-sequence.sh          # tiempo real
//   bash ./demos/amiga/107_xlimited_corkscrew/analyze-sequence.sh --warp   # rápido
//
...
//
// bash ./tools/build/build-demo.sh demos/amiga/107_xlimited_corkscrew --release --clean
// bash ./tools/run/run-demo.sh demos/amiga/107_xlimited_corkscrew --config A500_release
   
// bash ./tools/run/run-demo.sh demos/amiga/107_xlimited_corkscrew --config A500_release --sequence-frames 40 --sequence-interval-ms 250
