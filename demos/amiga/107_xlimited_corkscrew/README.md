...
EXTRA_DEFINES="-DK_EFFECT=3" bash ./tools/build/build-demo.sh demos/amiga/107_xlimited_corkscrew --debug --clean

# Solo vertical arriba (K_EFFECT=4) — composición inversa (columna/fila entrante por arriba)
EXTRA_DEFINES="-DK_EFFECT=4" bash ./tools/build/build-demo.sh demos/amiga/107_xlimited_corkscrew --debug --clean

# Solo horizontal izquierda (K_EFFECT=2) — pre-scroll derecho y composición inversa
EXTRA_DEFINES="-DK_EFFECT=2" bash ./tools/build/build-demo.sh demos/amiga/107_xlimited_corkscrew --debug --clean

# DPF 3+3 (dos capas): PF1 transparente sobre PF2, ambos scrollean
EXTRA_DEFINES="-DK_DUAL=1" bash ./tools/build/build-demo.sh demos/amiga/107_xlimited_corkscrew --debug --clean

# DPF 3+3 con parallax en X (PF2 a media velocidad)
EXTRA_DEFINES="-DK_DUAL=1 -DK_PARALLAX=1" bash ./tools/build/build-demo.sh demos/amiga/107_xlimited_corkscrew --debug --clean

# Ciclo completo empezando por V arriba (fase 3) y 500 frames por fase
EXTRA_DEFINES="-DK_START_PHASE=3 -DK_PHASE_FRAMES=500" bash ./tools/build/build-demo.sh demos/amiga/107_xlimited_corkscrew --debug --clean

# Tiles de 32 píxeles (misma altura, doble anchura, fuerza 384)
EXTRA_DEFINES="-DK_TILE_WIDTH=32" bash ./tools/build/build-demo.sh demos/amiga/107_xlimited_corkscrew --debug --clean

# Fetch ancho 32 px (BPL32, 384 px, DDF $28/$C8, offset 16, scroll 32)
EXTRA_DEFINES="-DK_FETCH_MODE=1" bash ./tools/build/build-demo.sh demos/amiga/107_xlimited_corkscrew --debug --clean

# Fetch ancho 32 px + tiles 32 px (384 px, 24 bloques por fila)
EXTRA_DEFINES="-DK_TILE_WIDTH=32 -DK_FETCH_MODE=1" bash ./tools/build/build-demo.sh demos/amiga/107_xlimited_corkscrew --debug --clean

# Fetch ancho 64 px (BPL32+BPAGEM, 384 px, DDF $18/$B8, offset 48, scroll 64)
EXTRA_DEFINES="-DK_FETCH_MODE=3" bash ./tools/build/build-demo.sh demos/amiga/107_xlimited_corkscrew --debug --clean
```
...
- **Análisis visual**: `bash ./tools/analyze/analyze-demo.sh demos/amiga/107_xlimited_corkscrew`
  o `analyze-sequence.sh` si existe.
- **Secuencia corkscrew** (`demos/amiga/107_xlimited_corkscrew/analyze-sequence.sh`):
  ...
  bash ./demos/amiga/107_xlimited_corkscrew/analyze-sequence.sh
  ```
  ...
  **Validación por dirección (`EFFECT` env)**: compila la demo con `K_EFFECT=$EFFECT`,
  ...
  EFFECT=4 bash ./demos/amiga/107_xlimited_corkscrew/analyze-sequence.sh --warp
  # Scroll vertical abajo / horizontal izquierda
  EFFECT=3 bash ./demos/amiga/107_xlimited_corkscrew/analyze-sequence.sh --warp
  EFFECT=2 bash ./demos/amiga/107_xlimited_corkscrew/analyze-sequence.sh --warp
  ```
...
- `demos/amiga/107_xlimited_corkscrew/src/main.cpp` — **consumidor fino**: define el
