...
# y valida con analyze_fine_scroll.py (numpy).
...
PY="$(dirname "${BASH_SOURCE[0]}")/analyze_fine_scroll.py"
...
invoke_case() {
	...
	if ! python3 "$PY" "$SEQ_DIR" "$RUN_REPORT" "$camera_x" "$expected_shifts"; then
