...
$sequenceDir = Join-Path $root "out\run\052_tile_staging_blits\sequence"
$pixelAssertOut = Join-Path $root "out\analysis\052_tile_staging_blits\pixel-assert"
...
function Invoke-RunCapture {
	...
	throw "No se pudo capturar la secuencia de 052_tile_staging_blits."
}
...
if ($LASTEXITCODE -ne 0) {
	throw "La secuencia de 052_tile_staging_blits no cumple las comprobaciones base."
}
...
if ($PixelAssert -or $RequirePixelAssertOk) {
	...
	if ($LASTEXITCODE -ne 0) {
		throw "Pixel Assertions detecto desviacion en 052_tile_staging_blits."
	}
}
