...
// de la carga. Ver docs/debugging/system/debug-winuae-v2-guide.md y run_status.hpp.
extern "C" {
...
// al invertir la dirección. Ver docs/engine/architecture/AMIGA_8WAY_SCROLLING.md §13.
//
...
//   EXTRA_DEFINES="-DK_VIEWPORT_W=288 -DK_VIEWPORT_H=224" bash ./tools/build/build-demo.sh demos/techniques/amiga/playfield/107_xlimited_corkscrew --debug --clean
//
...
//   bash ./tools/run/run-demo.sh demos/techniques/amiga/playfield/107_xlimited_corkscrew
//
//   # Capturar N frames de secuencia cada M ms (análisis de scroll):
//   bash ./tools/run/run-demo.sh demos/techniques/amiga/playfield/107_xlimited_corkscrew --sequence-frames 100 --sequence-interval-ms 20
//
//   # Asentamiento previo (ms reales tras READY) y warp (throughput, no suavidad):
//   bash ./tools/run/run-demo.sh demos/techniques/amiga/playfield/107_xlimited_corkscrew --settle-ms 500 --warp
//
//   # Dejar WinUAE corriendo al terminar:
//   bash ./tools/run/run-demo.sh demos/techniques/amiga/playfield/107_xlimited_corkscrew --keep-running
//
// -----------------------------------------------------------------------------
// CÓMO VERIFICAR
// -----------------------------------------------------------------------------
//   # Secuencia completa de la demo (H derecha + saveword + columna plane-shifted):
//   bash ./demos/techniques/amiga/playfield/107_xlimited_corkscrew/analyze-sequence.sh          # tiempo real
//   bash ./demos/techniques/amiga/playfield/107_xlimited_corkscrew/analyze-sequence.sh --warp   # rápido
//
//   # Análisis de artefactos de la captura:
//   bash ./tools/analyze/analyze-demo.sh demos/techniques/amiga/playfield/107_xlimited_corkscrew
//
...
//
// bash ./tools/build/build-demo.sh demos/techniques/amiga/playfield/107_xlimited_corkscrew --release --clean
// bash ./tools/run/run-demo.sh demos/techniques/amiga/playfield/107_xlimited_corkscrew --config A500_release
   
// bash ./tools/run/run-demo.sh demos/techniques/amiga/playfield/107_xlimited_corkscrew --config A500_release --sequence-frames 40 --sequence-interval-ms 250
...
// Pre-scroll para fases con componente negativa (izquierda/arriba) o diagonales.
    ...
    void update(eng::amiga::AmigaBackend& backend, eng::GameContext& context) {
        ...
        // Telemetría (F1): byte0=mapposx · byte1=fila del bucle donde cayó ink · byte2=d
        // (display_offset) · byte3=0x10|técnica · bit31=¿se pintó en lo visible?
        ...
        const eng::u32 marker = (f.dbg_ink_visible() ? 0x80000000u : 0u) |
            ...
            (static_cast<eng::u32>(f.dbg_ink_visible_row() & 0xff) << 8) |
            (static_cast<eng::u32>(f.display_offset() & 0xff) << 16);
