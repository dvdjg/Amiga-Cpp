...
real del Copper en `docs/guides/roadmap/XLIMITED_8WAY_EHB_201.md`.
...
punto de partida es el atlas **`assets/amiga/tiles-reference/Beginning Fields.png`**
...
```
 Beginning Fields.png (640x640, 40x40 tiles 16x16)
        ...
        ▼
 out/assets/ehb/palette.json                   (32 bases RGB) + ehb_palette.h + preview
        │  ②  slice-tiles.mjs           (cuantiza, dedupe 1149 tiles, reindexa BASES-PRIMERO)
        ▼
 out/assets/ehb/tilebank.raw.bin               (índices EHB 0..63, 256 B/tile, 1149 tiles)
 out/assets/ehb/tilebank_indexed.h             (paleta + mapa kTileIndexedMap + stride)
 out/assets/ehb/tiles.json                     (metadatos: banco, mapa, stats)
 out/assets/ehb/reconstruct.png / tilebank.png (verificación host)
        │  ③  emit-const-201.mjs        (renombra .h -> const_game_201.h)
        ▼
 out/assets/ehb/const_game_201.h               (kEhbPalette[64] + kRenderMap[1600])
        │  ④  emit-xlimited-bank.mjs    (índices -> banco interleaved 320px)
        ▼
 out/assets/ehb/tilebank.xlimited.bin          (222,720 B, 6 planos interleaved, listo)
```

La demo incrusta:
- `const_game_201.h` mediante `#include "../../../../out/assets/ehb/const_game_201.h"` → paleta
  `kEhbPalette` y mapa `kRenderMap` (los 40×40 índices al banco),
- `out/assets/ehb/tilebank.xlimited.bin` mediante `incbin` en la sección `tiles.MEMF_CHIP`
...
chipset. Ver la regla 7 de `docs/guides/roadmap/REGLAS_PIPELINE_TILES.md`.
...
```bash
node tools/ehb/quantize-ehb.mjs assets/amiga/tiles-reference/Beginning\ Fields.png --out out/assets/ehb
```
...
`out/assets/ehb/palette.json` (32 bases), `ehb_palette.h` (palabras Amiga `0x0RGB`) y
...
```bash
node tools/ehb/slice-tiles.mjs assets/amiga/tiles-reference/Beginning\ Fields.png \
  --palette out/assets/ehb/palette.json \
  --out out/assets/ehb
```
...
```bash
node tools/ehb/emit-const-201.mjs --in out/assets/ehb/tilebank_indexed.h \
  --json out/assets/ehb/tiles.json --out out/assets/ehb/const_game_201.h
```
...
> a los commiteados en `out/assets/ehb/`, y `slice-tiles.mjs` reporta `COMPARAR=100.00%` y
...
Los resultados de `out/assets/tile-demos/amiga/07_tiles_32x32/atlas_32x32` muestran ese banco generado a
...
bash ./tools/run/run-demo.sh demos/techniques/amiga/playfield/201_ehb_map [--sequence-frames N]
bash ./tools/analyze/analyze-demo.sh demos/techniques/amiga/playfield/201_ehb_map
```
