...
#   bash demos/techniques/amiga/playfield/202_xlimited_dpf/analyze-sequence.sh [--warp] [--config A500_debug]
...
bash ./tools/build/build-demo.sh demos/techniques/amiga/playfield/202_xlimited_dpf "--$([ "$CONFIG" = A500_release ] && echo release || echo debug)" --clean >/dev/null

echo "[202] run + secuencia..."
bash ./tools/run/run-demo.sh demos/techniques/amiga/playfield/202_xlimited_dpf --config "$CONFIG" --sequence-frames 3 --sequence-interval-ms 150 "${WARP[@]}" >/dev/null

echo "[202] verificación (ambas capas en movimiento continuo)..."
node ./demos/techniques/amiga/playfield/202_xlimited_dpf/verify-parallax.mjs --config "$CONFIG"

echo "[202] regresión Y independiente (warp, settle largo)..."
bash ./tools/run/run-demo.sh demos/techniques/amiga/playfield/202_xlimited_dpf --config "$CONFIG" --warp --settle-ms 55000 --sequence-frames 0 >/dev/null
node - "$CONFIG" <<'NODE'
const fs = require('fs');
