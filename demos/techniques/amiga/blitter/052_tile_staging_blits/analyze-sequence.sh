...
DEMO="demos/techniques/amiga/blitter/052_tile_staging_blits"
...
SEQ_DIR="$ROOT/out/run/052_tile_staging_blits/A500_debug/sequence"
PIXEL_OUT="$ROOT/out/analysis/052_tile_staging_blits/pixel-assert"
...
run_capture() {
  ...
  echo "No se pudo capturar la secuencia de 052_tile_staging_blits." >&2
  exit 1
}

run_capture
if ! "$SEQ_ANALYZER" "$SEQ_DIR" --expect-static; then
  echo "La secuencia de 052_tile_staging_blits no es estable tras el frame validado." >&2
  exit 1
fi
if [ "$PA" -eq 1 ]; then
...
echo "OK 052_tile_staging_blits sequence"
