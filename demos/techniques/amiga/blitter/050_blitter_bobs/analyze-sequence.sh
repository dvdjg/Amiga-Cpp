...
SEQ_DIR="$ROOT/out/run/050_blitter_bobs/A500_debug/sequence"
PIXEL_OUT="$ROOT/out/analysis/050_blitter_bobs/pixel-assert"
...
run_capture() {
  ...
  echo "No se pudo capturar la secuencia de 050_blitter_bobs." >&2
  exit 1
}

run_capture
if ! "$SEQ_ANALYZER" "$SEQ_DIR" --expect-static; then
  echo "La secuencia de 050_blitter_bobs no es estable tras el frame validado." >&2
  exit 1
fi
if [ "$PA" -eq 1 ]; then
...
echo "OK 050_blitter_bobs sequence"
