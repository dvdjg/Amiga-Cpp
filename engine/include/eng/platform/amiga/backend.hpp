...
/// \file backend.hpp
/// Backend Amiga (`eng::amiga::AmigaBackend`) del engine.
///
...
/// El vocabulario de chipset Amiga vive en `eng/platform/amiga/` y el modelo de capas
/// en docs/engine/architecture/PLATFORM_LAYERS.md.
///
/// Politica actual:
...
public:
	...
	static void fill_slot(ServiceSlot& slot, Service<C> fn, C& ctx) {
		slot.thunk = &AmigaBackend::service_thunk<C>;
	...
	void wait_vblank(Service<C> task, C& ctx) {
		...
		wait_vblank_run(&AmigaBackend::direct_thunk<C>, &token);
	}
	...
	/// `eng/core/data/rtc.hpp` y `amiga-bootcamp/01_hardware/common/cia_chips.md`.
