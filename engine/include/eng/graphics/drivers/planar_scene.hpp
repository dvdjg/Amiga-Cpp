...
/// \file planar_scene.hpp
/// Driver reutilizable para displays **planares (N bitplanes) con repeticion de filas**.
///
/// Cubre tanto modos HAM (nace del porte 1:1 de `effects/fire-rgb`: un buffer *chunky*
/// que un C2P pasa a bitplanes, y el display es HAM6 con **cuadruplicado de lineas** por
/// Copper, cada fila logica repetida variando `BPL1MOD`/`BPL2MOD`, mas un `BPLCON1`
/// alterno) como displays planares normales (4 planos y `row_repeat = 1`, p. ej. las
/// demos 081/116).
///
...
///
/// El nombre historico de este driver era `HamScene`; se renombro a `PlanarScene`
/// porque el alcance es planar generico, no el modo HAM. Se conservan los alias
/// `HamScene`/`HamSceneConfig` al final del fichero por compatibilidad.
...
struct PlanarSceneConfig {
	...
	/// Planos de bitplane (4 = 16 colores, 6 = HAM6/EHB...).
	u8 planes = 6;
	...
	/// Paleta opcional cargada al principio de la lista (paleta plana o base HAM).
...
/// Para doble buffer se crean varias instancias (una por buffer), cada una con su
/// propia copperlist, o se usa `MultiBuffered<PlanarScene, N>`.
class PlanarScene {
public:
	static constexpr GraphicsDriverId id = GraphicsDriverId::PlanarScene;
	...
	bool bind(eng::Block<eng::PlaneTag> bitplanes, eng::Block<eng::CopperTag> copper,
		  const PlanarSceneConfig& config) {
		m_config = config;
	...
	bool rebuild_copper(const PlanarSceneConfig& config) {
...
/// Alias de compatibilidad con el nombre historico.
using HamSceneConfig = PlanarSceneConfig;
using HamScene = PlanarScene;
