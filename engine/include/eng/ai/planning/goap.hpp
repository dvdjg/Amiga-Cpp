...
/// \file goap.hpp
/// **Planificacion GOAP** (Goal-Oriented Action Planning, `eng::ai`) en plantillas:
/// el agente describe el mundo con **hechos booleanos** y las acciones como
/// **precondiciones + efectos + coste**, y el planificador busca con A* la secuencia
/// de acciones de coste minimo que lleva del estado actual al objetivo.
///
...
/// ## Modelo
///
/// - `WorldState<MaxFacts>`: hasta `MaxFacts` hechos booleanos (`BitSet`). Cada hecho
///   es un indice (`Fact`); su significado (p. ej. "disk1_en_C", "horno_caliente") lo
///   decide el juego con un `enum`.
/// - `Action<MaxFacts>`: hechos requeridos a 1 (`require`), requeridos a 0 (`forbid`),
///   hechos que pone a 1 (`produce`), hechos que pone a 0 (`consume`) y `cost`.
/// - `Goal<MaxFacts>`: hechos que deben estar a 1 (`want_true`) y a 0 (`want_false`).
/// - `Planner<MaxFacts, MaxActions, MaxNodes>`: A* hacia delante con heuristica de
///   **objetivos pendientes** (hechos del objetivo sin cumplir).
///
/// ## Uso
///
...
///   } };
///   eng::ai::Planner<8, 3, 64> planner;
///   eng::u16 plan[8];
///   eng::ai::Goal<8> meta;
///   meta.want_true.facts.set(kHorneado);
///   const eng::usize n = planner.plan(eng::ai::make_state<8>(),
///                                     meta, acciones.span(), eng::Span<eng::u16> {plan, 8});
///
/// ## Coste y limites (68000)
///
/// - Pensado para **`init` o tareas de fondo**, no para el camino por frame: reserva un
///   `HashMap` y una `PriorityQueue` de capacidad `MaxNodes` mas los nodos generados.
///   Para un presupuesto comodo en A500, instanciar el `Planner` en memoria estatica
///   (no en la pila) y dimensionar `MaxNodes` por escenario. `expansions()` dice cuanto
///   trabajo costo el ultimo plan.
/// - `MaxFacts <= 32`: la clave de busqueda del estado se empaqueta en un `u32` (una
///   palabra del `BitSet`), lo que hace la deduplicacion `O(1)` sin hashes largos.
/// - La heuristica cuenta hechos del objetivo pendientes. Es admisible cuando cada accion
///   satisface como mucho un hecho del objetivo (el caso tipico de estos planes); si una
///   accion resolviera varios, la heuristica puede sobreestimar y el plan no seria
///   estrictamente optimo (seguiria siendo valido). Con coste unitario y un solo hecho
///   objetivo degenera en busqueda de anchura; el presupuesto `MaxNodes` la acota.
...
/// Indice de hecho invalido (raiz del arbol de busqueda).
inline constexpr eng::u16 no_fact_link = 0xffffu;

using eng::u32;
using eng::u16;
using eng::usize;

/// Conjunto de hechos booleanos de un agente: `MaxFacts` bits.
template <usize MaxFacts>
struct WorldState {
	static_assert(MaxFacts > 0u && MaxFacts <= 32u,
		      "WorldState: de 1 a 32 hechos (la clave se empaqueta en u32)");
	...
	/// Clave densa del estado (unica hasta 32 hechos) para deduplicar en la busqueda.
...
template <usize MaxFacts, class... Fs>
[[nodiscard]] constexpr WorldState<MaxFacts> make_state(Fs... facts) noexcept {
	WorldState<MaxFacts> s {};
...
template <usize MaxFacts>
struct Action {
	WorldState<MaxFacts> pre_true {};  ///< hechos que deben estar a 1
	WorldState<MaxFacts> pre_false {}; ///< hechos que deben estar a 0
	WorldState<MaxFacts> eff_add {};   ///< hechos que la accion pone a 1
	WorldState<MaxFacts> eff_del {};   ///< hechos que la accion pone a 0
	u16 cost = 1u;
	const char* name = nullptr; ///< solo diagnostico/host; puede ser `nullptr`
};

/// Constructor fluido de acciones (se usa en `constexpr` en los escenarios).
template <usize MaxFacts>
class ActionBuilder {
public:
	...
	template <u16... Facts>
	[[nodiscard]] constexpr ActionBuilder& require() noexcept {
		((m_action.pre_true.facts.set(Facts)), ...);
		return *this;
	}
	template <u16... Facts>
	[[nodiscard]] constexpr ActionBuilder& forbid() noexcept {
		((m_action.pre_false.facts.set(Facts)), ...);
		return *this;
	}
	template <u16... Facts>
	[[nodiscard]] constexpr ActionBuilder& produce() noexcept {
		((m_action.eff_add.facts.set(Facts)), ...);
		return *this;
	}
	template <u16... Facts>
	[[nodiscard]] constexpr ActionBuilder& consume() noexcept {
		((m_action.eff_del.facts.set(Facts)), ...);
		return *this;
	}
...
template <usize MaxFacts>
struct Goal {
	WorldState<MaxFacts> want_true {};
	WorldState<MaxFacts> want_false {};
};
...
template <usize MaxFacts>
[[nodiscard]] constexpr bool applicable(const WorldState<MaxFacts>& s,
...
template <usize MaxFacts>
constexpr void apply(WorldState<MaxFacts>& s, const Action<MaxFacts>& a) noexcept {
	using Word = typename eng::util::BitSet<MaxFacts>::word_type;
	constexpr usize W = eng::util::BitSet<MaxFacts>::word_count;
	for (usize w = 0; w < W; ++w) {
...
template <usize MaxFacts>
[[nodiscard]] constexpr bool satisfies(const WorldState<MaxFacts>& s,
...
template <usize MaxFacts>
[[nodiscard]] constexpr u16 goal_distance(const WorldState<MaxFacts>& s,
					  const Goal<MaxFacts>& g) noexcept {
	using Word = typename eng::util::BitSet<MaxFacts>::word_type;
	constexpr usize W = eng::util::BitSet<MaxFacts>::word_count;
	u16 d = 0u;
...
template <usize MaxFacts, usize MaxActions, usize MaxNodes = 128u>
class Planner {
	static_assert(MaxActions > 0u, "Planner: MaxActions debe ser mayor que 0");
...
public:
	...
	[[nodiscard]] constexpr usize plan(const WorldState<MaxFacts>& start, const Goal<MaxFacts>& goal,
					   Span<const Action<MaxFacts>> actions, Span<u16> out) noexcept {
		m_found = false;
		m_cost = 0u;
		m_expansions = 0u;
		m_node_count = 0u;
		...
		if (actions.empty() || out.empty() || actions.size() > MaxActions) {
			return 0u;
		}
		...
		while (!m_open.empty()) {
			...
			for (usize ai = 0; ai < actions.size(); ++ai) {
				const Action<MaxFacts>& a = actions[ai];
				if (!applicable(state, a)) {
					continue;
				}
				WorldState<MaxFacts> next = state;
				apply(next, a);
