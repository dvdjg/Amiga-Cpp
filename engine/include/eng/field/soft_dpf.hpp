...
public:
	...
	eng::u8* display_base() const { return m_view.display_base(); }
	eng::u8* write_base() const { return m_view.write_base(); }
	...
	bool init(eng::MemorySystem& memory, const eng::gfx::BitmapConfig& bc,
	          eng::gfx::Bitmap& main) {
		m_view.bind_single(main.allocation_start(), main.bytes().data());
	...
	eng::graphics::BlitJob make_copy_rect_job(const eng::u8* pattern, u16 pattern_row_bytes,
		...
		const eng::u8* src = pattern + static_cast<eng::u32>(src_y) * pat_row + bs.word_bytes;
		eng::u8* dst_base = m_view.write_base();
	...
	eng::graphics::BlitJob make_copy_job(const eng::u8* pattern, u16 pattern_row_bytes,
