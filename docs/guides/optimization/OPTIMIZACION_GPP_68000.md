...
| 2026-09-14 | **Solape CPU/Blitter** (flatshade-convex) | [✓] El original lanza el `BitmapClearFast` **sin esperar** (se solapa con el transform CPU). `blitter_clear`/`blitter_area_fill` ahora aceptan `wait=false` y se expone `MinimalBackend::wait_blitter()`; lanzar clear y fill sin esperar sube 116 de 10.6 a **14.3 fps** (el fill de 1024 líneas, ~143k ciclos de bus, se solapa con la espera de VBlank). Un blit de pantalla completa sin solapar cuesta ~todo un frame. |
