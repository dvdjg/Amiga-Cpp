...
| 2026-09-14 | **Solape CPU/Blitter y buffer a medias** (flatshade-convex) | [✓] El original lanza el `BitmapClearFast` **sin esperar** (se solapa con el transform CPU); `blitter_clear`/`blitter_area_fill` aceptan `wait=false` y se expone `MinimalBackend::wait_blitter()`. Pero solapar el **fill** que se va a mostrar **parpadea**: el buffer se muestra a mitad de relleno (frames con hasta 16 % de huecos internos). Regla: solapar un blit con la CPU **solo si escribe un buffer que NO se está mostrando** (el clear del buffer trasero); el fill del buffer que se muestra debe esperarse (`WaitBlitter` antes del swap, como el original). Neto: 10.6 → **16.7 fps** con la imagen estable. |
...
| 2026-09-14 | **BLITHOG mal activado en un intento previo** | [✓] El intento previo usaba `dma_blithog = 0x0080`, que es `DMAF_COPPER`, NO `DMAF_BLITHOG` (0x0400, ver `<hardware/dmabits.h>`). Nunca activó BLITHOG. La vía correcta es `MinimalBackend::set_blitter_priority(true)` (`DMACON = 0x8400` = SETCLR\|BLTPRI). Con BLITHOG el fill baja de 143.8k a 131.8k (igual que el original). |
...
| 2026-09-15 | **Port asm de flatshade-convex: render negro resuelto (25 fps)** | [V] Con `K_FLATSHADE_ASM=1` la demo pasa `verify-116` y el frame baja a **283k ciclos (25.0 fps)** frente a ~20.7 fps de la ruta C++. Tres causas encontradas y corregidas: (1) **cámara pisada**: en `fs_update_face_visibility` la `camera.z` se cargaba en `d4`, reutilizado luego como temporal en `v`/`mag2`, así que `pz` salía mal desde la 2ª cara; se recargan `cx/cy/cz` por cara. (2) **`.Lwait_blit` pisaba `d0`**: la espera de BBUSY cargaba DMACONR en `d0`, que `fs_draw_edges` tiene como BLTCON0 y escribe justo después del wait → se programaba DMACONR como con0 y **ningún blit de línea pintaba** (síntoma: balón ausente sin crashear, `nEdges=0`). Se salva/restaura `d0`. (3) **`BLTAPT` sin extender**: `move.l d3` dejaba la palabra alta de `derr` con basura; el acumulador de error del modo línea es de 32 bits y hay que escribir `derr` extendido con signo (`ext.l`), como `blitter_line_eor_draw`. Añadidos además `blt_signflag` cuando `derr<0` (como la ruta C++ del engine) y el `DMACON` al inicio. Método: aislar con draw C++ + transform asm (PASS) → el fallo está en `fs_draw_edges`. |
