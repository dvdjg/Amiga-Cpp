...
| 2026-09-14 | **Coste de una escritura a registro custom** | [✓] Micro-benchmark: un blit de **1 palabra** cuesta **1,404 ciclos** con `cpu_cycle_exact`; es decir, todo blit tiene un coste fijo (~1,400) y **cada escritura a registro custom ~57 ciclos**. Por eso fusionar los 3 punteros `BLTxPT` en **una escritura de 32 bits** (`write_custom_pointer`) y fijar los comunes **1×/frame** (`blitter_lines_begin`) baja la línea de 1,752 a **1,400** y los edges de 129k a **109k**. Regla de diseño: minimizar el numero de escrituras a registros custom y de arranques de blit por frame. |
