...
**Override Amiga del hook por Blitter — HECHO**: el relleno de polígonos se delega al Blitter sin que el llamador cambie.
...
- **Validación en hardware**: demo **`111_xlimited_sidescroller`** — un cubo sólido (3 caras sombreadas, malla `constexpr`, `mesh_render_filled`) se rellena por Blitter sobre el lienzo FG **interleaved** del DPF (`detail` bits 27..24 = caras dibujadas). El borrado del recuadro también va por Blitter (el `fill_rect` CPU de 88×88 costaba ~0,4 M ciclos/frame).

Pendiente: **animar** el cubo por frame (el lienzo FG es de un solo buffer: borrado+redibujado cada frame provoca flicker; requiere doble buffer del lienzo FG o dibujar a un buffer y blit) e importar `flatshade`/`stencil3d` encima. Nota: el Blitter de `amiga_minimal` cobra ~57 ciclos por escritura a registro custom y cada blit espera al anterior, así que el número de blits por cara domina el coste.
