...
Pendiente: **override Amiga del hook por Blitter** (necesita un `Playfield` con acceso al backend o un job de `FramePlan`; el backend ya expone `blitter_fill_polygon`, falta el puente al hook + scratch `mask` Chip + variante interleaved), luego reintroducir la **demo cabecera** en hardware, e importar `flatshade`/`stencil3d` encima. La **capa de bobs/personajes** es la opción A del roadmap general (110/111 ya a medio pulir).
