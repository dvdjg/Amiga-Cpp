...
| `docs/guides/roadmap/ROADMAP_CARD_GAMES.md` | **vigente (C0–C4 hechos; C5 juego pendiente)** | Motores de naipes (`eng::cards`): póker **Texas Hold'em No-Limit** con baraja determinista, reglas (ciegas, calles, acciones legales, botes laterales y showdown), evaluador de 5/7, equity Monte Carlo, IA por estilos con modelo de rival y simulación CPU vs CPU, verificado por HOST-161…165 con **footprint `N20`…`N512`**. Falta el juego con UI en `games/` (API **NO VERIFICADA** en hardware). Diseño en `docs/engine/architecture/CARD_GAME_AI.md` |
