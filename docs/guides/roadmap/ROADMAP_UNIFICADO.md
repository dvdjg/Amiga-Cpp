...
- **Ecosistema vivo (`eng::sim`) — modelo base entregado** y verificado por test host:
  ...
  **131 ticks/s** solo y **113 frames/s** con planificación realista (~1.2x); el peor caso sin
  caché era ~18x, y se cerró usando la **caché de planes** en el driver y el **intervalo de
  replan**. Destapó y corrigió el libcall `__popcountsi2` (ahora SWAR freestanding). Falta el
