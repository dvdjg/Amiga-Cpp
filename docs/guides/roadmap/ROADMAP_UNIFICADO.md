...
Pendiente: **override Amiga del hook por Blitter** (necesita un `Playfield` con acceso al backend o un job de `FramePlan`), **demo cabecera** del subsistema en hardware, e importar `flatshade`/`stencil3d` encima. La **capa de bobs/personajes** es la opción A del roadmap general (110/111 ya a medio pulir).
