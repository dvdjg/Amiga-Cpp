...
|-----------|-------------|
| [octamed-startmusic-hang.md](octamed-startmusic-hang.md) | **A1 (reencuadrado con la repro `274_octamed_probe`)**: `_startmusic` **retorna**, pero la demo **cuelga antes del frame 30**; descartada la ruta de interrupción del playroutine (`VBLANK=0,CIAB=0` también cuelga) y el bucle `_Wait1line` (`$dff007` sí cambia). Incluye un bug latente real en `_IntHandler` (`A1` vs `is_Data`). Siguiente: GDB/pasos. |
