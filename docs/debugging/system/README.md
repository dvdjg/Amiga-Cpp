# Sistema de depuración WinUAE-DBG

Cómo está montado y cómo **operar** el sistema de depuración (Windows + VS Code/Cursor + WinUAE-DBG con gdbserver y canal lateral). Proceden del repo hermano `Cursor-Amiga-C` (engine en C) y son **bitácoras**: pueden citar nombres de API de versiones anteriores (`div16`/`mul16`). El estado vigente del engine está en `docs/engine/`.
