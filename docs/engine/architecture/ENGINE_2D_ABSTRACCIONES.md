...
```
   ...
   │   -> Span<u8>             │           │ amiga_backend (direcciones)   │
...
- **Especialización por dominio**: sobre `Span`, el engine define tipos de dominio (`Bytes<Tag>`/`Words<Tag>`, unidades fuertes, direcciones/base) para que un error de dominio no compile (patrón ≠ audio, base ≠ front, ancho ≠ alto). Fundamento y auditoría: `INTERNAL_TYPE_SYSTEM.md` (`eng/core/types/typed.hpp`).
