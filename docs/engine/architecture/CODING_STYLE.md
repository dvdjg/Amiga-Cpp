...
- **Sin wrappers elementales ni nombres de caso concreto**: no crear funciones «preset» que solo rellenan 2-3 campos de un struct de configuración o que empaquetan una llamada trivial (`planar4`, `canvas`, `ham`, `ehb`, `make_*` de una línea). Desde fuera sugieren implementaciones distintas y son *vanilla*; además **reducen** la comprensión. Preferir **una función paramétrica** reutilizable y bien documentada; si hay escenarios de uso que destacar, ponerlos en **comentarios de uso** (doc-comment con ejemplos), no como funciones nuevas. Regla general: **los nombres de métodos y clases deben ser genéricos y describir el mecanismo, no una configuración concreta**. Batida de deuda y detalle: `docs/debugging/investigaciones/pending-verification.md` §1.
- **El medio/la configuración es un DATO, no una especialización**: no crear una clase por banco de memoria, modo o plataforma cuando el **mecanismo es el mismo** (p. ej. `ChipPool` → `BlockPool`, que opera sobre cualquier buffer y lleva su `MemoryKind` en cada reserva; el medio Chip/Fast/Slow es un parámetro, no un tipo). Antes de crear una clase, preguntar **«¿qué varía aquí?»**: si varía el medio/escalar/configuración, **parametrizar** (plantilla `Tag`/`S`/política) en vez de duplicar N clases casi idénticas. Misma regla que el escalar: **un mecanismo, un nombre genérico**; lo concreto viaja como dato o política.
- **Ante un `cast`, revisar el tipo de origen**: una conversión (`static_cast`/`reinterpret_cast` o construir un tipo fuerte desde un `void*`) es un indicio de **tipo de dato mal elegido**. Antes de forzarla, comprobar si el origen **garantiza** lo que el destino afirma; si no, cambiar el tipo o la **procedencia** del dato, nunca forzar el cast. Ejemplo: un `Address<MemoryKind::Chip>` solo se obtiene de una fuente cuya procedencia es Chip —una arena/`MemBank<MemoryKind::Chip>`, un `gfx::Bitmap` (siempre Chip) o un búfer en `.MEMF_CHIP`—; construirlo con un cast desde un array de pila/Fast **oculta el error** que el sistema de tipos debía revelar. El puente desde una dirección de almacenamiento es `Address<K>::from_storage(ptr)`, y solo es lícito donde la procedencia está garantizada (backend o almacén tipado); un test host no tiene Chip RAM, así que ahí no se prueba el medio. Si necesitas una tabla **estática** en Chip, usa `eng::ChipStorage<Tag, N>` declarado con `ENG_CHIP_RAM` (`eng/memory/chip_storage.hpp`): coloca el búfer en `.MEMF_CHIP` y entrega la `Address<Chip>`/vista **sin cast**; para assets, `INCBIN_CHIP` (`support/gcc8_c_support.h`). **Preferible a reservarla dinámicamente.**
- **Todo `cast` debe demostrar que hace falta**: antes de dejar un `static_cast`/`reinterpret_cast`, **quitar el cast y compilar con las mismas banderas** (`-Wall -Wextra -Werror=narrowing`). Si no aparece aviso y el destino ya admite el valor, el cast es **ruido** (repite una conversión implícita que el lenguaje ya hace) y se elimina; deja solo el que **cambia semántica** (narrowing en *list-init* `T{…}`, `void*`→tipo, truncado intencionado) o el que **documenta una frontera**. Clave: los argumentos y las copias (`T x = expr;`, `f(expr)`) **no** disparan `-Wnarrowing` —solo la inicialización de lista—, así que ahí casi nunca hace falta; deja que el compilador convierta y reserva el cast para cuando el valor entra en un agregado `T{…}` o para conversiones de punteros. Un cast que no silencia un aviso real **oculta** el tipo mal elegido en vez de señalarlo (complementa la regla anterior). Ejemplo real: `static_cast<s16>(a.x - fine_scroll)` como `const s16 fx = …` sobra (promoción `int`→`s16` implícita); el mismo valor dentro de `PRect { …, }` sí necesita el cast (narrowing de list-init).
- **Operaciones de memoria básicas con *builtins*, no a mano**: para rellenar o copiar buffers usa `__builtin_memset`/`__builtin_memcpy`/`__builtin_memmove` (o `memset`/`memcpy`/`memmove`, provistos por `support/gcc8_c_support.c`) en vez de bucles `for` elemento a elemento. **No son STL** y la restricción freestanding **no** los prohíbe; con `-O2` GCC los incentiva (inlinea el tamaño constante y usa el camino a palabra alineada en variable), mejor que un bucle manual. Reserva la aritmética de punteros a mano para casos con **alineación o rendimiento justificados** (p. ej. escribir `u32` en planos alineados) y coméntalo. La *serialización* con formato (campos empaquetados) sigue usando los cursores seguros de `binary.hpp`, no `memcpy`.
- **El target Amiga se detecta con `ENG_AMIGA`, no con `__m68k__`**: el build de Amiga define `-DENG_AMIGA=1` (`tools/build/build-demo.sh`). `__m68k__`/`__mc68000__` solo indican **CPU** m68k (también lo son Mega Drive y Atari ST, que **no** tienen Chip RAM ni el periférico de depuración de WinUAE): úsalos solo para **codegen** (instrucciones, endianness, aritmética `mulu16`…). Para lo específico de Amiga (Chip RAM, `debug_*`, Exec, `0xB7xxxx`) gatea con `ENG_AMIGA`.
...
- **Regla de oro de genericidad de cabeceras**: una cabecera debe ser **lo más genérica posible**. Sólo las de una **implementación concreta** específica de hardware (`eng/retro/`, `eng/platform/`, `eng/cpu/`) o del **propio escalar** (`fixed*.hpp`, `minifloat*.hpp`) pueden nombrar representaciones concretas (`Fixed<s16,…>`/`Fixed<s32,…>`, `q0/q8/q12/q24`, `MiniFloat16`). Un algoritmo **no fija el escalar**: lo recibe por plantilla (`S`) y pide lo que necesite por los **puntos de extensión** (`scalar_traits`, `scalar_div`, `scalar_const`, `numeric_traits`, `noise_traits`, `scalar_sin`/`scalar_cos`…, `mesh_traits`), que el escalar especializa en **su** cabecera. Ejemplo: `mesh3d` se plantilla sobre el escalar de coordenada y el culling 16 bits (`muls.w`) vive en `eng/retro/fixed_mesh.hpp`. **Lo que no pueda ser genérico se decopla como política de plantilla** (parámetro de tipo), no se hardcodea en el algoritmo: p. ej. la fase amplia de vecinos del crowd es `Crowd<S, Broadphase>` (`eng/ai/steering/crowd.hpp`), con `SpatialHashBroadphase` (rejilla `s16`) o `BruteForceBroadphase` (cualquier escalar) como políticas. El gate `tools/check/generic-headers.mjs` lo verifica en cada pasada (con baseline de la deuda histórica en `generic-headers-baseline.txt`); el resumen operativo está en `AGENTS.md` §1.10.
