...
- **Ante un `cast`, revisar el tipo de origen**: una conversión (`static_cast`/`reinterpret_cast` o construir un tipo fuerte desde un `void*`) es un indicio de **tipo de dato mal elegido**. Antes de forzarla, comprobar si el origen **garantiza** lo que el destino afirma; si no, cambiar el tipo o la **procedencia** del dato, nunca forzar el cast. Ejemplo: un `Address<MemoryKind::Chip>` solo se obtiene de una fuente cuya procedencia es Chip —una arena/`MemBank<MemoryKind::Chip>`, un `gfx::Bitmap` (siempre Chip) o un búfer colocado con `ENG_CHIP_RAM`—; construirlo con un cast desde un array de pila/Fast **oculta el error** que el sistema de tipos debía revelar. El cast como «frontera explícita» solo es lícito donde la procedencia está garantizada (backend o almacén tipado). Si necesitas una tabla fija en Chip, colócala con `ENG_CHIP_RAM` (sección `.MEMF_CHIP`), **preferible a reservarla dinámicamente**.
