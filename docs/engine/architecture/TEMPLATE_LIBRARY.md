...
- **Matemáticas `Fixed`**: `eng/core/fixed_math.hpp` especializa `scalar_sin`/`scalar_cos`/`scalar_sqrt`/`scalar_exp2`/`scalar_log2`/`scalar_exp`/`scalar_log`/`scalar_pow` para `Fixed<s16,E>` (tablas compartidas + `isqrt`); incluir ese header antes de usar easings `_sine`/`_expo`, `smooth_damp`, `length`/`normalize`/`project`/`reflect` o `stddev` con fixed. `sin`/`cos` requieren `E <= 14`; `exp2` satura; `log2` solo correcto dentro del rango del fixed. Tablas: 2 KiB (seno) + 1 KiB (`exp2`) + 1 KiB (`log2`) por exponente, compartidas y con **tamaño elegible en compilación** (`fixed_sin<E,Size,Iter>`, `fixed_exp2<E,Size>`, `fixed_log2<E,Size>`; p. ej. 1 KiB en total con seno 256 y exp2/log2 64). Verificado por HOST-104 y **por demo** (`110_ylimited_shooter`, self-test en `init`).
