...
| [DISPLAY_COMPOSITION.md](DISPLAY_COMPOSITION.md) | **Contrato de buffers y copper**: las tres granularidades (display / un plano / solo copperlist), `scene::compose` con `SceneResources.buffers` como capa de buffers, el modo `SceneMode::CopperChunky` (sin bitplanes) y el supervisor `copper::Plan`. |
...
| [PUBLIC_GAME_API.md](PUBLIC_GAME_API.md) | **API de juego (borrador evolutivo)**: `eng::App`/`eng::Screen` (`eng/api/game.hpp`) sobre lo que ya existe, con el mapeo desde el API interno; regla prioritaria: intuitivo, simple y sin restringir funcionalidad. |
| [ROADMAP_API_COHERENCE.md](ROADMAP_API_COHERENCE.md) | **Coherencia de la API**: diagnóstico de incongruencias (puerta única, `App` como *service locator*, pila de dibujo solapada, propiedad de la caché, nombres duplicados, memoria/errores) y **plan por fases** (F0–F6) hacia una API de juego elegante. |
| [EFFECT_MODEL.md](EFFECT_MODEL.md) | **Modelo de efectos (diseño)**: contrato `Effect`, familias `DisplayEffect`/`RenderEffect`, composición en la escena, propiedad de banda y presupuesto por efecto. |
...
| [RESOURCE_SYSTEM.md](RESOURCE_SYSTEM.md) | **Recursos** (`eng::res`): caché de assets con presupuesto, prioridad, refcount y desalojo LRU sobre la E/S asíncrona; **DynLoader** de código relocatable (`.englib`) y fachada `Resources`. Plan en `../../guides/roadmap/ROADMAP_RESOURCES.md`. |
...
| [VISUAL_EFFECT_SPRITE_DESIGN.md](VISUAL_EFFECT_SPRITE_DESIGN.md) | Parte gráfica/efectos del diseño: `Visual`, `CopperIntent`, `HwSpriteTemplate` y concept `Effect`; mapa de layers de display, simetría actor/playfield. |
...
| [GAME_AUDIO.md](GAME_AUDIO.md) | Capa de audio de juego (`GameAudio`/`SampleBank`): política de voces y ducking, **modos de audio y reparto de canales** (`AudioMode`), integración con el mini-SO y guía para generar música y sonidos desde herramientas externas. |
| [AUDIO_STREAMING.md](AUDIO_STREAMING.md) | **Streaming digital desde disquete**: compresión (ZX0/aPLib/Delta+RLE) y formato AUZX, doble/triple buffer en Chip, IRQ de Paula y descompresión en tarea de fondo. |
...
| [MINI_OS_MESSAGE_LOOP.md](MINI_OS_MESSAGE_LOOP.md) | **Mini-SO de mensajes** (`eng::os` + `eng::ui`): puerto de mensajes IRQ-safe, señales, productores (VBlank/input/disco/timers), prioridad, **VBlank latched** y despacho por tabla; bucle de aplicación reactivo sin espera activa. Plan en `../../guides/roadmap/ROADMAP_MINI_OS.md`. |
| [MINI_OS_INPUT.md](MINI_OS_INPUT.md) | **Mini-SO: entrada de hardware → mensajes**: ratón (`JOY0DAT`+CIA/POTINP), joystick (`JOY1DAT`), pad CD32 (`POTGO`/`POTINP`) y teclado (CIA-A serie), según el AHRM; reutiliza `input_poll`/`InputAggregator`. |
| [MINI_OS_TIME.md](MINI_OS_TIME.md) | **Mini-SO: tiempo y timers**: fuentes (VBlank/CIA/TOD/beam), `TickClock` de µs sobre CIA-B, `ScopedTimer`, `TimerService` (frames/µs) → `MsgType::Timer` y one-shot de CIA. |
| [MINI_OS_IO.md](MINI_OS_IO.md) | **Mini-SO: E/S asíncrona y streaming**: `dos.library`/`trackdisk`, `IoNotify`, `FileDone`/`FileError`, doble/triple buffer para audio desde disquete y footprint. |
...
| [GOAP_EXTENDED.md](GOAP_EXTENDED.md) | **GOAP ampliado** (`eng::ai`+`eng::sim`): estado híbrido hechos+niveles, acciones con `target` y coste dinámico, *anytime* con presupuesto, caché con invalidación selectiva, plan coordinado y HTN ligero. Diseño objetivo (adopción en G7). |
