...
| [engine/](engine/README.md) | Diseño del engine C++ actual: estilo, drivers gráficos, memoria, política de hardware/ROM, benchmarks de API, IA de juego (`engine/architecture/GAME_AI_LIBRARY.md`), ecosistema vivo (`engine/architecture/SIM_ECOSYSTEM.md`), motores de tablero (`engine/architecture/BOARD_GAME_AI.md`), motores de naipes (`engine/architecture/CARD_GAME_AI.md`), concurrencia abstracta (`engine/architecture/PARALLEL_AND_THREADS.md`) y roadmap (en `engine/architecture/`) + histórico del engine C (`engine/c-engine/`). |
