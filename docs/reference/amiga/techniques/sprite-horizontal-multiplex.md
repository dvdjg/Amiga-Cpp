...
- **Estado en el engine:** la **capa de fondo** completa está en `effects::SpriteLayer` (demo `207_sprite_layer`, validada). El patrón correcto (de `spr_layer/`, Jeroen Knoester) es **un `WAIT` al inicio de cada línea** y luego una **ráfaga** de `SPRxPOS`+`SPRxDATB`+`SPRxDATA` de **todos** los canales (el `SPRxCTL` se fija una vez por banda). Un `WAIT` por canal en su X **no** funciona (solo se arman algunos canales). `graphics::SpriteHorizontalRearm` + `Scheduler::emit_sprite_horizontal_rearm` cubren el caso de un rearm suelto; resumen del subsistema: [sprite-layer.md](sprite-layer.md) §11.
