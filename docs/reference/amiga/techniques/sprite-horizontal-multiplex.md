...
- **Estado en el engine:** `SpriteIntent` modela la **tira horizontal de canales contiguos** (`strip_id`/`strip_index`/`strip_span`, ver `engine/include/eng/graphics/raster_intent.hpp`). El **rearmado horizontal del mismo canal** está **implementado**: `graphics::SpriteHorizontalRearm` + `Scheduler::emit_sprite_horizontal_rearm` (`emit_sprite_horizontal_rearms` para la misma línea) y el intent `CopperIntentKind::SpriteRearm`. El **vertical** + color multiplexing los cubre `SpriteManager::emit_template_into`. Resumen del subsistema: [sprite-layer.md](sprite-layer.md) §11.
