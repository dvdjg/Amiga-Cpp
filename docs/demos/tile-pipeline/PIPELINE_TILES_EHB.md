...
(regla 7 de `docs/guides/roadmap/REGLAS_PIPELINE_TILES.md`).
...
# 1) Cuantizador EHB (32 bases + half). -> out/assets/ehb/palette.json
node tools/ehb/quantize-ehb.mjs "$SOURCE" --out out/assets/ehb
...
node tools/ehb/emit-const-201.mjs   # -> out/assets/ehb/const_game_201.h
...
comprometidos en `out/assets/ehb` (SHA256 de `palette.json`, `tilebank.raw.bin`, `tiles.json`,
`tilebank_indexed.h`, `const_game_201.h`, `tilebank.xlimited.bin`, `tilebank.xlimited.h`,
`reconstruct.png`, `tilebank.png` y `ehb_palette.h`). Es decir: el `out/assets/ehb` actual se
...
convierte los `gid` al índice de banco (`out/assets/ehb/tmx_bf.json` existe). Esta vía sirve
...
- **Datos (host, concluyente)**: `reconstruct.png` montado con `banco[kRenderMap]`
  ...
  regeneración completa es byte-idéntica al `out/assets/ehb` comprometido.
...
- **Runtime (parcial)**: la captura real muestra la paleta EHB esperada y el segmento
  ...
  `docs/guides/roadmap/XLIMITED_8WAY_EHB_201.md`.