...
| `palette.h` (`HsvToRgb`/`ColorTransition`/`FadeBlack`) | HECHO (helpers + efectos) | `eng::util::color.hpp`: `hsv_to_rgb444` (de `HsvToRgb`) y, sobre `Span<u16>`, `palette_lerp` (de `ColorTransition`), `palette_scale` (de `FadeBlack`) y `gradient444` (degradado multi-parada), con `muls.w`/`divs.w` y sin `float`. Efectos: `PaletteTransitionEffect` (`effects/palette_transition.hpp`, fundido; demo 040 lo compone, demo 030 encendido/apagon) y `RasterGradientEffect` (`effects/raster_gradient.hpp`, degradado por banda como `CopperIntent` para el `copper::Plan`; demo 085 lo usa en el cielo). Verificado por HOST-094/108/109, sonda `c_palette_ops` (0 libcalls) y las demos 030/040/085. |
