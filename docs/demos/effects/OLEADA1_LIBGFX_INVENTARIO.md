...
| `palette.h` (`HsvToRgb`/`ColorTransition`/`FadeBlack`) | HECHO (helpers puros) | `eng::util::color.hpp`: `hsv_to_rgb444` (de `HsvToRgb`) y, sobre `Span<u16>`, `palette_lerp` (transicion, de `ColorTransition`) y `palette_scale` (fundido a negro, de `FadeBlack`), con `muls.w`/`divs.w` y sin `float`. Verificado por HOST-094 y sonda `c_palette_ops` (0 libcalls). |
