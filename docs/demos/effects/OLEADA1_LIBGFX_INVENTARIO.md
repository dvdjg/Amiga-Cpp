...
| `palette.h` (`HsvToRgb`/`ColorTransition`/`FadeBlack`) | HECHO (helpers + efecto) | `eng::util::color.hpp`: `hsv_to_rgb444` (de `HsvToRgb`) y, sobre `Span<u16>`, `palette_lerp` (transicion, de `ColorTransition`), `palette_scale` (fundido a negro, de `FadeBlack`) y `gradient444` (degradado multi-parada, base de un futuro `RasterGradientEffect`), con `muls.w`/`divs.w` y sin `float`. El efecto `eng::graphics::effects::PaletteTransitionEffect` (`effects/palette_transition.hpp`) los integra en el formato de efectos de copper (`Effect<..., FramePlan>` + parche base o de zona); demo 040 lo compone con el ciclo y demo 030 lo usa para encendido/apagon. Verificado por HOST-094, HOST-108, sonda `c_palette_ops` (0 libcalls) y las demos 030/040. |
