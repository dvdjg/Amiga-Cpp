...
Cambios aplicados en esta iteración: `xy` con `math2d::mul16` (evita un `__mulsi3` por vértice: `transform_vertices` 57k→46k); **`blitter_lines_begin`** (los 6 comunes del modo línea se fijan 1× por frame); y **`write_custom_pointer` en una sola escritura de 32 bits** (los punteros BLTxPT se escribían en dos palabras). Medido con micro-benchmarks (`-DFLATSHADE_BENCH_LINE=1`): **cada acceso a registro custom cuesta ~57 ciclos** con `cpu_cycle_exact`, y **cualquier blit tiene un coste fijo ~1,400** (una escritura de 1 palabra tarda 1,404). Resultado: per-line 1,752→**1,400**, edges 129k→**109k**, `update` 383k→**353k**. El `fill` (143k) no baja (domina el número de words). El fps no cambia (429k = 3 vblanks) por la cuantización: hay que bajar el `update` a ~284k.
