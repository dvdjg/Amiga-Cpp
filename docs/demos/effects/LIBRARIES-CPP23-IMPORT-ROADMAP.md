...
| `lib3d` | ✅ | ✅ (matrices + caras + malla) | ✅ (host HOST-011/013 + demo 077) | ✅ | 06, 30, 56, 65 | `engine/include/eng/core/math3d.hpp`: `Mat3x3` (fixed-point 4.12) con `load_identity`/`translate`/`scale`/`load_rotate` (Rx·Ry·Rz)/`load_reverse_rotate` (Rz·Ry·Rx)/`compose`/`transform`, más `Face` + `face_visible` (back-face culling por signo), `face_z_sum`/`face_z_min` (claves del painter's algorithm). Reutiliza la tabla de seno de `math2d`. Validado por `tests/host/011_math3d`. Modelo de malla en `engine/include/eng/core/mesh3d.hpp`: `MeshView` (vértices + caras), `mesh_transform` (lote) y `mesh_painter_order` (culling + orden lejos→cerca por shell sort in-place, sin asignación), validado por `tests/host/013_math3d_mesh`. **Pendiente menor**: caras de doble cara (`AllFacesDoubleSided`) y formato binario de malla de `obj2c` (al consumirlo la capa UAF-R). |
| `libgui` | ❌ | ❌ | ❌ | ❌ | 37, 38 | `eng::ui` |
