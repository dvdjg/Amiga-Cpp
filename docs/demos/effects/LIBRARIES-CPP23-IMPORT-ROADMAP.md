...
| `lib3d` | ✅ | 🔄 (matrices + caras) | ✅ (host HOST-011) | 🔄 | 06, 30, 56, 65 | `engine/include/eng/core/math3d.hpp`: `Mat3x3` (fixed-point 4.12) con `load_identity`/`translate`/`scale`/`load_rotate` (Rx·Ry·Rz)/`load_reverse_rotate` (Rz·Ry·Rx)/`compose`/`transform`, más `Face` + `face_visible` (back-face culling por signo), `face_z_sum`/`face_z_min` (claves del painter's algorithm). Reutiliza la tabla de seno de `math2d`. Validado por `tests/host/011_math3d`. **Pendiente**: modelo de objeto/malla (`NewObject3D`, `UpdateObjectTransformation`, `UpdateVertexVisibility`, `AllFacesDoubleSided`) — atado al formato de malla binario de `obj2c` y al allocador; conviene cerrarlo cuando exista la capa `eng::assets` (UAF-R, `ENGINE_DESIGN.md` §5 paso 8). |
| `libgui` | ❌ | ❌ | ❌ | ❌ | 37, 38 | `eng::ui` |
