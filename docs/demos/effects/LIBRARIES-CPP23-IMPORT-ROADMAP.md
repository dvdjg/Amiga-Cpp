...
| `lib3d` | ✅ | 🔄 (matrices) | ✅ (host HOST-011) | 🔄 | 06, 30, 56, 65 | `engine/include/eng/core/math3d.hpp`: `Mat3x3` (fixed-point 4.12) con `load_identity`/`translate`/`scale`/`load_rotate` (Rx·Ry·Rz)/`load_reverse_rotate` (Rz·Ry·Rx)/`compose`/`transform`. Reutiliza la tabla de seno de `math2d`. Validado por `tests/host/011_math3d`. **Pendiente**: representación de malla/objeto (`NewObject3D`, `UpdateFaceVisibility`, `SortFaces`, `AllFacesDoubleSided`), que depende de allocador y `sort`. |
| `libgui` | ❌ | ❌ | ❌ | ❌ | 37, 38 | `eng::ui` |
