...
- **Los enlaces relativos de la documentación no deben quedar rotos**: `node tools/check/links.mjs` los valida y corre en `tools/test-regression.sh` y `tools/run-host-tests.sh`. La deuda histórica de los árboles importados (`docs/engine/c-engine`, `docs/legacy`) está aceptada en `tools/check/links-baseline.txt`; solo falla la rotura nueva (regenerar el baseline con `--update-baseline` al arreglar).
