...
- **API pública (la aplicación no ve hardware)**: `docs/engine/architecture/PUBLIC_API.md` — la app describe capas/cámaras/efectos y el planner deduce la composición; tipos fuertes, handles, errores sin excepciones, init/frame. Escena **retenida**, **modelo de ocupación** (headroom/can_add), introspección y representación de actores (sprite/BOB/playfield): `docs/engine/architecture/SCENE_AND_RESOURCES.md`.
