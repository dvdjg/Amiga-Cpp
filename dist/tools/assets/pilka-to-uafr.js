...
function initializerInts(text, name) {
    const m = text.match(new RegExp(`${name}\\[\\]\\s*=\\s*\\{([\\s\\S]*?)\\};`));
    if (!m)
        throw new Error(`no se encontro el array ${name}[]`);
    return (m[1].match(/-?\d+/g) ?? []).map(Number);
}
/** Offset (bytes) y count de un `eng::Span<eng::s16>{ (s16*)((char*)BLOB + OFF), CNT }`. */
function groupSpan(structBody, blob, name) {
    const re = new RegExp(`${name}\\s*=\\s*eng::Span<eng::s16>\\s*\\{\\s*` +
        `reinterpret_cast<eng::s16\\*>\\(\\(char\\*\\)${blob}\\s*\\+\\s*(\\d+)\\)\\s*,\\s*(\\d+)\\s*\\}`);
    const m = structBody.match(re);
    if (!m)
        throw new Error(`no se encontro ${name} en el struct`);
...
function convert() {
    ...
    if (!structMatch)
        throw new Error('no se encontro `Mesh3D pilka`');
    ...
    for (let k = 0; k < fg.count; k++) {
        ...
        for (let j = 0; j < count; j++) {
            ...
            if (vi === undefined)
                throw new Error(`offset de vértice desconocido: ${voff}`);
            face.push(vi);
        }
        faces.push(face);
    }
    ...
    console.log(`pilka: ${verts.length} vértices, ${faces.length} caras (n-gon), ${tris} triángulos ` +
