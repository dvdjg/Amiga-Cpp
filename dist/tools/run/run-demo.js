...
function parseProtectSpecs() {
    ...
    if (envSpecs) {
        for (const part of envSpecs.split(/\s+/))
            specs.push(part);
    }
    ...
    for (const spec of specs) {
        ...
        if (!target)
            throw new Error(`--protect target vacio: ${spec}`);
        ...
        if (![8, 16, 32].includes(size))
            throw new Error(`--protect size debe ser 8|16|32: ${spec}`);
...
async function captureFrameSequence(protocol, sequenceDir, frameCount, intervalMs, sideChannel = null, onSample = null) {
    ...
    for (let i = 0; i < frameCount; ++i) {
        ...
        if (sideChannel?.runtimeAddress) {
            try {
                runStatusBefore = await readSideChannelRunStatusOnce(sideChannel.port, sideChannel.runtimeAddress, sideChannel.timeoutMs ?? 1000);
            }
            catch (err) {
        ...
        if (sideChannel?.runtimeAddress) {
            try {
                frame.runStatusAfter = await readSideChannelRunStatusOnce(sideChannel.port, sideChannel.runtimeAddress, sideChannel.timeoutMs ?? 1000);
            }
            catch (err) {
 ...
 */
async function withSideChannelLock(port, mode, owner, fn) {
...
async function captureFrameSequenceByRunStatusTarget(protocol, sequenceDir, targets, sideChannel, selector, label) {
    ...
    while (targetIndex < targets.length && Date.now() <= deadline) {
        const runStatus = await readSideChannelRunStatusOnce(sideChannel.port, sideChannel.runtimeAddress, sideChannel.timeoutMs ?? 1000);
        ...
        if (value === targets[targetIndex] && frameNumber !== lastCapturedFrame) {
            ...
            try {
                ...
                await sleep(120);
                frozen = await readSideChannelRunStatusOnce(sideChannel.port, sideChannel.runtimeAddress, sideChannel.timeoutMs ?? 1000);
            }
            catch (err) {
...
async function captureFrameSequenceStep(protocol, sequenceDir, frameCount, sideChannel, probeSymbol, startFine = null) {
    ...
    try {
        while (captured < frameCount && attempts < maxAttempts) {
            ...
            const frozen = await readSideChannelRunStatusOnce(sideChannel.port, sideChannel.runtimeAddress, sideChannel.timeoutMs ?? 1000);
            ...
            const framePath = path.join(sequenceDir, `frame_${String(frames.length).padStart(3, '0')}_f${String(frameNumber).padStart(4, '0')}.png`);
            const capturedAt = Date.now();
            const frame = await captureScreenshot(protocol, framePath);
            frame.capturedAtMs = capturedAt;
        ...
        try {
            await protocol.clearBreakpoint(probeSymbol);
        }
...
try {
    ...
    if (keysToInject.length > 0) {
        console.log('[run-demo] aviso: `--keys` usa el monitor `input key` (mapeo `256+sc`, cae en `SPC_*`) y no ' +
    ...
    if (keyScanArg !== '') {
        ...
        if (!mm || addr === null) {
            ...
            for (const p of pairs) {
                console.log(`[run-demo] key-scan: id ${p.id} (0x${p.id.toString(16)}) -> rawkey 0x${p.raw
                    .toString(16)
                    .padStart(2, '0')}`);
            }
    ...
    if (sequenceCameraX.length > 0) {
        ...
        report.sequence = await captureFrameSequenceByRunStatusTarget(protocol, path.join(outputDir, 'sequence'), sequenceCameraX, {
            ...
            timeoutMs: 1000,
        }, decodeCameraX, 'cameraX');
    }
    else if (sequenceFineX.length > 0) {
        console.log(`[run-demo] capturing sequence by fineX targets ${sequenceFineX.join(',')}`);
        report.sequence = await captureFrameSequenceByRunStatusTarget(protocol, path.join(outputDir, 'sequence'), sequenceFineX, {
            port: sideChannelPort,
            runtimeAddress: report.sideChannel?.runtimeAddress,
            timeoutMs: 1000,
        }, decodeCameraFineX, 'fineX');
    }
    else if (sequenceStepFrames > 0) {
        console.log(`[run-demo] capturing ${sequenceStepFrames} frame-step sequence frames (1 frame apart)`);
        report.sequence = await captureFrameSequenceStep(protocol, path.join(outputDir, 'sequence'), sequenceStepFrames, {
            port: sideChannelPort,
            runtimeAddress: report.sideChannel?.runtimeAddress,
            timeoutMs: 1000,
        }, readyProbeSymbol, sequenceStepStartFine);
    }
