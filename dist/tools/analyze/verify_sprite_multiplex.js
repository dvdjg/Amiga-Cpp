...
function classify(r, g, b) {
    ...
    if (max < 128)
        return null; // fondo navy y negros: demasiado oscuros
    const t = 2;
    if (r >= g * t && r >= b * t)
        return 'red';
    if (g >= r * t && g >= b * t)
...
function main() {
    ...
    if (!imagePath)
        fail('Uso: verify_sprite_multiplex.js --image <png>');
    const resolved = path.resolve(imagePath);
    if (!fs.existsSync(resolved))
        fail(`No existe: ${resolved}`);
    const img = readPng(resolved);
    const hues = ['red', 'green', 'blue', 'yellow', 'cyan', 'magenta'];
    const count = Object.fromEntries(hues.map((h) => [h, 0]));
    const sumY = Object.fromEntries(hues.map((h) => [h, 0]));
    ...
    for (let i = 1; i < hues.length; i++) {
        if (!(centroids[i] > centroids[i - 1])) {
            fail(`FAIL sprite: franjas no ordenadas verticalmente (${hues[i]}@${centroids[i].toFixed(0)} no va después de ${hues[i - 1]}@${centroids[i - 1].toFixed(0)})`);
        }
    }
