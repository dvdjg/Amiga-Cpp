...
import { readPng, createImage, savePng, pixel, setPixel } from '../lib/image.js';
function readJson(filePath) {
    return JSON.parse(fs.readFileSync(filePath, 'utf-8'));
}
function listFrames(sequenceDir) {
...
function detectNonBlackViewport(image, threshold = 8) {
...
function resolveViewport(contract, firstFrame) {
...
function resolveCoordinateSpace(contract, viewport) {
...
function clampRoi(roi, viewport, frameW, frameH, coords) {
...
function pixelDiffExceeds(a, b, tolerance) {
...
function directionCompatible(observed, expected) {
    if (expected === 'static')
        return observed === 'static';
    const parts = expected.split('-');
    return parts.every((part) => observed.includes(part));
}
function shiftedRegionErrorRatio(a, b, roiAbs, dx, dy, tolerance) {
...
function writeMismatchOverlay(a, b, roiAbs, dx, dy, tolerance, outPath) {
    const [x0, y0, w, h] = roiAbs;
    if (w <= 0 || h <= 0)
        return 0;
...
function bestShiftForRoi(a, b, roiAbs, tolerance, searchRadius) {
...
function forbiddenColorRatio(image, roiAbs, color, tolerance) {
    const [x0, y0, w, h] = roiAbs;
    if (w <= 0 || h <= 0)
    ...
    for (let y = y0; y < y0 + h; ++y) {
        for (let x = x0; x < x0 + w; ++x) {
            ...
            if (Math.abs(p[0] - color[0]) <= tolerance &&
...
function writeForbiddenOverlay(image, roiAbs, color, tolerance, outPath) {
    const [x0, y0, w, h] = roiAbs;
    if (w <= 0 || h <= 0)
        return 0;
...
function pairRange(segment, frameCount) {
...
function loadTelemetry(runReportPath) {
...
function runAssertions(frames, contract, viewport, coords, runReportPath, overlaysDir) {
    ...
    for (const check of contract.globalChecks ?? []) {
        ...
        if (consideredFrames.length === 0)
            consideredFrames = perFrame;
        let maxSeen = 0.0;
    ...
    for (const segment of contract.segments ?? []) {
        ...
        for (const check of segment.checks ?? []) {
            ...
            if (consideredPairs.length === 0)
                consideredPairs = pairResults;
            ...
            if (!passed) {
                if (ctype === 'telemetry_direction_match') {
                    for (const pairItem of pairResults) {
                        if (pairItem.compatible)
                            continue;
                        const overlayName = `${segmentName}_${ctype}_f${String(pairItem.from).padStart(3, '0')}_f${String(pairItem.to).padStart(3, '0')}.png`;
                        const overlayPath = path.join(overlaysDir, overlayName);
                        writeMismatchOverlay(frameImages[pairItem.from], frameImages[pairItem.to], roiAbs, parseInt(pairItem.observedDx, 10), parseInt(pairItem.observedDy, 10), tolerance, overlayPath);
                ...
                for (const pairItem of pairResults) {
                    if (pairItem.from < start + ignoreFirstPairs)
                        continue;
                    if (pairItem.errorRatio <= maxErrorRatio)
                        continue;
                    const overlayName = `${segmentName}_${ctype}_f${String(pairItem.from).padStart(3, '0')}_f${String(pairItem.to).padStart(3, '0')}.png`;
                    const overlayPath = path.join(overlaysDir, overlayName);
                    writeMismatchOverlay(frameImages[pairItem.from], frameImages[pairItem.to], roiAbs, parseInt(pairItem.dx ?? 0, 10), parseInt(pairItem.dy ?? 0, 10), tolerance, overlayPath);
                    pairItem.overlay = overlayPath;
                }
                failures.push({
                    name: entry.name,
...
function buildSummary(report, sequenceDir, contractPath) {
    const lines = [];
