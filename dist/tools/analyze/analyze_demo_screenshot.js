...
function sampleColors(imagePath) {
    ...
    const counts = { width, height, white: 0, dark: 0, nonBlue: 0, green: 0, yellow: 0 };
...
function checkRunReport(imagePath, expectedDemo) {
...
function main() {
    ...
    const failures = [];
    if (counts.white < minWhite)
        failures.push(`white=${counts.white} < ${minWhite}`);
    if (counts.dark < minDark)
        failures.push(`dark=${counts.dark} < ${minDark}`);
    if (counts.nonBlue < minNonBlue)
        failures.push(`nonBlue=${counts.nonBlue} < ${minNonBlue}`);
    if (needGreen && counts.green < 1)
        failures.push(`green=${counts.green} < 1`);
    if (needYellow && counts.yellow < 1)
        failures.push(`yellow=${counts.yellow} < 1`);
