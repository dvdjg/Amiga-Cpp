...
async function main() {
    ...
    try {
        ...
        report.copper = { cop1lc: null, head: null };
        ...
        if (copResp.ok) {
            ...
            if (headResp.ok)
                report.copper.head = String(headResp.data);
        }
        ...
        assertOk(sc.status === 0, `screendump-diff --from-copper must succeed; status=${sc.status} stderr=${sc.stderr?.slice(0, 400)}`);
