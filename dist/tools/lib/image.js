...
export function setPixel(image, x, y, r, g, b, a = 255) {
    const i = (y * image.width + x) * 4;
    image.data[i] = r;
    image.data[i + 1] = g;
    image.data[i + 2] = b;
...
export function createImage(width, height, fill = [0, 0, 0]) {
...
export function savePng(image, outPath) {
...
export function copyRect(src, dst, sx, sy, dx, dy, w, h) {
    for (let y = 0; y < h; ++y) {
...
export function drawText(image, x, y, text, color) {
