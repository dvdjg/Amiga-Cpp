...
function findMapSymbol(mapPath: string, symbolName: string): number | null {
...
function findMapAllocSections(mapPath: string): Array<{ name: string; start: number; end: number }> {
...
function resolveRuntimeSymbolAddress(
  linkedSymbol: number,
  mapSections: Array<{ start: number; end: number }>,
  runtimeSections: any,
): number | null {
...
class SideChannelClient {
  ...
  async command(cmd: string, timeoutMs = 2000): Promise<any> {
...
async function runBuild(demo: string): Promise<void> {
...
function parseHexBytes(hex: unknown): Buffer {
...
function decodeReport(buf: Buffer): { magic: number; version: number; count: number; failed: number; checks: number; cases: MathCase[] } {
...
async function main(): Promise<void> {
  ...
  const port = parseInt(argValue('--port', process.env.WINUAE_SIDE_CHANNEL_PORT || '2346') as string, 10);
  const readyTimeoutMs = parseInt(argValue('--ready-timeout-ms', '30000') as string, 10);
  const waitMs = parseInt(argValue('--wait-ms', '12000') as string, 10);
  ...
  const runner = spawn(
    'bash',
    ['tools/run/run-demo.sh', demo, '--wait-ms', String(waitMs), '--settle-ms', '1200', '--side-channel-timeout-ms', '6000'],
  ...
  try {
    ...
    const runtime = resolveRuntimeSymbolAddress(reportSymbol as number, mapSections, state.sections);
