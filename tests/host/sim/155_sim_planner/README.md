...
2. **`PlanRunner`**: cargar un plan, consumir pasos por tick (`current`/`advance`),
   terminar (`done`) y abortar; un plan que no cabe en `MaxSteps` se rechaza.
3. **`PlannerDriver`**: replanificar con `Goap` (plan óptimo de tres acciones encadenadas,
...
4. **Dominio de construcción** (`domain.hpp`): desde cero, conseguir refugio exige
