...
static_assert((q12 {4096} * q12 {2048}).norm<12>().narrow<s16>() == q12 {2048},
...
static_assert(q12 {8192}.norm<0>() == q0 {2}, "4.12 -> entero (trunca");
static_assert(to_int(q12 {8192}) == 2, "to_int");

// Mezclar exponentes NO compila (se comprueba que la expresión no es válida).
template <typename A, typename B>
concept Sumable = requires(A a, B b) { a + b; };
template <typename A, typename B>
concept Mulable = requires(A a, B b) { a * b; };
static_assert(Sumable<q12, q12>, "4.12 + 4.12 si");
static_assert(!Sumable<q12, q0>, "4.12 + entero NO compila (exponentes distintos)");
static_assert(!Sumable<q12, Fixed<s16, 8>>, "4.12 + 8.8 NO compila (exponentes distintos)");
...
static_assert((q12 {4096} * q14 {16384}).norm<12>().narrow<s16>() == q12 {4096},
...
static_assert((Fixed<s32, 24> {8191}.norm<12>() + Fixed<s32, 24> {1}.norm<12>()).narrow<s16>() == q12 {1},
...
static_assert(q12 {8191}.norm<0>() == q0 {1}, "truncar hacia -inf (gratis)");
static_assert(q12_round {8191}.norm<0>().v == s16 {2}, "redondeo al mas cercano (1 add)");
static_assert(q12_round {2048}.norm<0>().v == s16 {1}, "0.5 -> 1 (half up)");
// Desbordamiento al estrechar: envolver vs saturar.
static_assert(Fixed<s32, 12> {200000}.narrow<s16>().v == s16 {3392}, "wrap (complemento a 2)");
static_assert(Fixed<s32, 12, SaturatePolicy> {200000}.narrow<s16>().v == s16 {32767}, "satura");
...
int main() {
	...
	check((a * b).norm<12>().narrow<s16>() == q12 {2048}, "0.5 en 4.12");
