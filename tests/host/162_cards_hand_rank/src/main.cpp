// ============================================================================
// Test HOST-162: evaluador de manos de poker (eng::cards/rules/hand_rank.hpp)
// ============================================================================
//
...
void test_best_of_seven() {
	...
	check(hand_category(evaluate_hand(seven, 7u)) == HandCategory::Flush,
	...
	check(hand_category(evaluate_hand(full7, 7u)) == HandCategory::FullHouse,
	      "7 cartas: full house");

	// Menos de 5 cartas: sin valor.
	const Card two[2] {c(Rank::Ace, Suit::Spades), c(Rank::King, Suit::Hearts)};
	check(evaluate_hand(two, 2u) == kHandValueNone, "menos de 5 cartas: sin valor");
}
