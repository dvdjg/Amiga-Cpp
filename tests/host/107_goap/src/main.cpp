...
template <usize F, usize A, usize N>
void run_and_check(const char* label, u16 expected_cost, usize expected_len,
		   const eng::ai::WorldState<F>& start, const eng::ai::Goal<F>& goal,
		   const eng::util::Array<eng::ai::Action<F>, A>& actions) {
	eng::ai::Planner<F, A, N> planner;
	u16 plan[32] {};
	...
	eng::ai::WorldState<F> s = start;
	for (usize i = 0; i < n; ++i) {
		const eng::ai::Action<F>& a = actions[plan[i]];
...
constexpr eng::util::Array<eng::ai::Action<9>, 18> make_hanoi_actions() {
	eng::util::Array<eng::ai::Action<9>, 18> acts {};
	usize k = 0;
	for (u16 d = 0; d < 3u; ++d) {
...
void test_hanoi() {
	...
	const auto start = eng::ai::make_state<9>(kDiskFact(0, 0), kDiskFact(1, 0),
						  kDiskFact(2, 0));
	eng::ai::Goal<9> goal {};
	...
	run_and_check<9, 18, 64>("hanoi", 7u, 7u, start, goal, kHanoiActions);
}
...
constexpr eng::util::Array<eng::ai::Action<8>, 8> kCakeActions { {
	eng::ai::ActionBuilder<8> {}.named("comprar_harina").cost(2u).produce<kHarina>().build(),
	eng::ai::ActionBuilder<8> {}.named("comprar_huevos").cost(2u).produce<kHuevos>().build(),
	eng::ai::ActionBuilder<8> {}.named("comprar_azucar").cost(1u).produce<kAzucar>().build(),
	eng::ai::ActionBuilder<8> {}.named("comprar_mantequilla").cost(1u).produce<kMantequilla>().build(),
	eng::ai::ActionBuilder<8> {}
		.named("batir")
		.cost(3u)
		.require<kHarina, kHuevos, kAzucar, kMantequilla>()
		.produce<kMezcla>()
		.build(),
	eng::ai::ActionBuilder<8> {}.named("precalentar").cost(4u).produce<kHornoCaliente>().build(),
	eng::ai::ActionBuilder<8> {}
		.named("hornear")
		.cost(5u)
		...
		.produce<kHorneado>()
		.build(),
	eng::ai::ActionBuilder<8> {}.named("decorar").cost(2u).require<kHorneado>().produce<kPastelListo>().build(),
} };

void test_cake() {
	const eng::ai::WorldState<8> start {};
	eng::ai::Goal<8> goal {};
	goal.want_true.facts.set(kPastelListo);
	// 2+2+1+1 (compras) + 3 (batir) + 4 (horno) + 5 (hornear) + 2 (decorar) = 20.
	run_and_check<8, 8, 64>("pastel", 20u, 8u, start, goal, kCakeActions);
}
...
constexpr eng::util::Array<eng::ai::Action<16>, 13> kSoldierActions { {
	eng::ai::ActionBuilder<16> {}.named("recoger_alicates").cost(1u).produce<kAlicates>().build(),
	eng::ai::ActionBuilder<16> {}.named("recoger_llave").cost(1u).produce<kLlave>().build(),
	eng::ai::ActionBuilder<16> {}.named("cortar_alambre").cost(3u).require<kAlicates>().produce<kAlambreCortado>().build(),
	eng::ai::ActionBuilder<16> {}.named("abrir_puerta").cost(1u).require<kLlave>().produce<kPuertaAbierta>().build(),
	eng::ai::ActionBuilder<16> {}.named("entrar_arsenal").cost(2u).require<kPuertaAbierta>().produce<kEnArsenal>().build(),
	eng::ai::ActionBuilder<16> {}.named("coger_rifle").cost(2u).require<kEnArsenal>().produce<kRifle>().build(),
	eng::ai::ActionBuilder<16> {}.named("coger_municion").cost(2u).require<kEnArsenal>().produce<kMunicion>().build(),
	eng::ai::ActionBuilder<16> {}.named("cargar_rifle").cost(3u).require<kRifle, kMunicion>().produce<kRifleCargado>().build(),
	eng::ai::ActionBuilder<16> {}.named("arrancar_generador").cost(4u).produce<kGenerador>().build(),
	eng::ai::ActionBuilder<16> {}.named("abrir_puerta_electrica").cost(1u).require<kGenerador>().produce<kPuertaElectrica>().build(),
	eng::ai::ActionBuilder<16> {}.named("avanzar_puesto").cost(3u).require<kAlambreCortado, kPuertaElectrica>().produce<kEnPuesto>().build(),
	eng::ai::ActionBuilder<16> {}.named("abatir_enemigo").cost(2u).require<kEnPuesto, kRifleCargado>().produce<kEnemigoAbatido>().build(),
	eng::ai::ActionBuilder<16> {}.named("cumplir_mision").cost(1u).require<kEnemigoAbatido>().produce<kObjetivo>().build(),
} };

void test_soldier() {
	const eng::ai::WorldState<16> start {};
	eng::ai::Goal<16> goal {};
	...
	run_and_check<16, 13, 256>("soldado", 26u, 13u, start, goal, kSoldierActions);
}
...
void test_already_satisfied() {
	const auto start = eng::ai::make_state<8>(kPastelListo);
	eng::ai::Goal<8> goal {};
	goal.want_true.facts.set(kPastelListo);
	eng::ai::Planner<8, 8, 32> planner;
	u16 plan[4] {};
...
void test_no_solution() {
	...
	eng::ai::Planner<8, 8, 32> planner;
	u16 plan[4] {};
	const usize n = planner.plan(eng::ai::WorldState<8> {}, goal, kCakeActions.span(),
...
void test_forbid() {
	...
	constexpr eng::util::Array<eng::ai::Action<8>, 3> acts { {
		eng::ai::ActionBuilder<8> {}.named("comprar_azucar").cost(1u).produce<kAzucar>().build(),
		eng::ai::ActionBuilder<8> {}
			.named("sin_azucar")
			.cost(2u)
	...
	eng::ai::Planner<8, 3, 32> planner;
	u16 plan[4] {};

	// Sin azucar: gana la accion que prohibe el azucar (coste 2, sin comprar).
	const usize n0 = planner.plan(eng::ai::WorldState<8> {}, goal, acts.span(),
				      eng::Span<u16> {plan, 4u});
	...
	const auto con_azucar = eng::ai::make_state<8>(kAzucar);
