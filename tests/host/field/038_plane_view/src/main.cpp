...
int main() {
	...
	const eng::BitmapBase mr {eng::Address<eng::MemoryKind::Chip>::from_storage(main_real)},
	                       er {eng::Address<eng::MemoryKind::Chip>::from_storage(extra_real)};
	const eng::FrontBase mf {eng::Address<eng::MemoryKind::Chip>::from_storage(main_front)},
	                      ef {eng::Address<eng::MemoryKind::Chip>::from_storage(extra_front)};
	...
	check(pv.display_base().value.cptr() == main_real && pv.write_base().value.cptr() == main_front, "single: bases");
	pv.flip();
	check(pv.display_base().value.cptr() == main_real && pv.write_base().value.cptr() == main_front, "single: flip no cambia");

	// Con doble buffer: front = principal, back = extra (active=0).
	pv.bind_raw(mr, mf, er, ef);
	check(pv.double_buffered(), "db: activo");
	check(pv.display_base().value.cptr() == main_real && pv.write_base().value.cptr() == extra_front,
	      "db active=0: front=main, back=extra");

	// flip: front = extra, back = principal.
	pv.flip();
	check(pv.display_base().value.cptr() == extra_real && pv.write_base().value.cptr() == main_front,
	      "db active=1: front=extra, back=main");
	// flip vuelve al estado inicial (toggle).
	pv.flip();
	check(pv.display_base().value.cptr() == main_real && pv.write_base().value.cptr() == extra_front, "db: toggle de vuelta");
